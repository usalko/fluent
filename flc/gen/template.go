	...
	Templates = []TypeTemplate{
		{
			Name:   "create",
			Cond:   notView,
			Format: package_name_format("%s_create.go"),
		...
		},
		{
			Name:   "update",
			Cond:   notView,
			Format: package_name_format("%s_update.go"),
		},
		{
			Name:   "delete",
			Cond:   notView,
			Format: package_name_format("%s_delete.go"),
		},
		{
			Name:   "query",
			Format: package_name_format("%s_query.go"),
			ExtendPatterns: []string{
		...
		{
			Name:   "model",
			Format: package_name_format("%s.go"),
		},
		{
			Name:   "where",
			Format: package_name_format("%s/where.go"),
			ExtendPatterns: []string{
				"where/additional/*",
			},
		},
...
type (
	...
	// use the flc.Dependency option in order to build this annotation.
	Dependency struct {
...
func (d *Dependency) Build() error {
	if d.Type == nil {
		return errors.New("flc/gen: missing dependency type")
	}
	if d.Field == "" {
...
func (d *Dependency) defaultName() (string, error) {
	...
	default:
		return "", fmt.Errorf("flc/gen: unexpected number of parts: %q", parts)
	}
...
func package_name_format(s string) func(t *Type) string {
