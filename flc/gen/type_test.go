...
func TestType(t *testing.T) {
	...
	typ, err := NewType(&Config{Package: "flc/gen"}, T1)
	require.NoError(err)
	require.NotNil(typ)
	...
	_, err = NewType(&Config{Package: "flc/gen"}, &load.Schema{
		Fields: []*load.Field{
	...
	_, err = NewType(&Config{Package: "flc/gen"}, &load.Schema{Name: "Type"})
	require.EqualError(err, "schema lowercase name conflicts with Go keyword \"type\"")
	_, err = NewType(&Config{Package: "flc/gen"}, &load.Schema{Name: "Int"})
	require.EqualError(err, "schema lowercase name conflicts with Go predeclared identifier \"int\"")
	_, err = NewType(&Config{Package: "flc/gen"}, &load.Schema{Name: "Value"})
	require.EqualError(err, "schema name conflicts with fluent predeclared identifier \"Value\"")
}
...
func TestType_Receiver(t *testing.T) {
	...
	for _, tt := range tests {
		typ := &Type{Name: tt.name, Config: &Config{Package: "flc/gen"}}
