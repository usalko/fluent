...
import (
	...
	fluent_schema "github.com/usalko/fluent/schema"
...
type (
	...
	Type struct {
		*Config
		schema *load.Schema
		// Name holds the type/fluent name.
		Name string
...
func (t Type) EntSQL() *fluent_sql.Annotation {
...
func (t *Type) AddIndex(idx *load.Index) error {
	...
	case len(ant.PrefixColumns) != 0 && ant.Prefix != 0:
		return fmt.Errorf("index %q cannot contain both fluent_sql.Prefix and fluent_sql.PrefixColumn in annotation", index.Name)
	case ant.Prefix != 0 && len(idx.Fields)+len(idx.Edges) != 1:
		return fmt.Errorf("fluent_sql.Prefix is used in a multicolumn index %q. Use fluent_sql.PrefixColumn instead", index.Name)
	case len(ant.PrefixColumns) > len(idx.Fields)+len(idx.Fields):
		return fmt.Errorf("index %q has more fluent_sql.PrefixColumn than column in its definitions", index.Name)
	}
...
// but are not exported (not defined with field). i.e. generated by fluent.
...
func (t Type) sqlComment() string {
	...
	ant := &fluent_schema.CommentAnnotation{}
...
// EntSQL returns the EntSQL annotation if exists.
func (f Field) EntSQL() *fluent_sql.Annotation {
...
var mutMethods = func() map[string]bool {
	...
	t := reflect.TypeOf(new(fluent.Mutation)).Elem()
...
// EntSQL returns the EntSQL annotation if exists.
func (e Edge) EntSQL() *fluent_sql.Annotation {
...
// sqlAnnotate extracts the fluent_sql.Annotation from a loaded annotation format.
func sqlAnnotate(annotation map[string]any) *fluent_sql.Annotation {
	annotate := &fluent_sql.Annotation{}
	if annotation == nil || annotation[annotate.Name()] == nil {
		return nil
	}
	if buf, err := json.Marshal(annotation[annotate.Name()]); err == nil {
...
// sqlIndexAnnotate extracts the fluent_sql annotation from a loaded annotation format.
func sqlIndexAnnotate(annotation map[string]any) *fluent_sql.IndexAnnotation {
	annotate := &fluent_sql.IndexAnnotation{}
	if annotation == nil || annotation[annotate.Name()] == nil {
		return nil
	}
	if buf, err := json.Marshal(annotation[annotate.Name()]); err == nil {
