...
// StorageDrivers holds the storage driver options for flc.
var drivers = []*Storage{
	{
		Name:      "sql",
		IdentName: "SQL",
	...
	},
	{
		...
		Imports: []string{
			...
			"github.com/usalko/fluent/dialect/gremlin/encoding/graphson",
		},
		SchemaMode: Unique,
...
// is here in order to remove the validation logic from flc command line.
func NewStorage(s string) (*Storage, error) {
	...
	return nil, fmt.Errorf("flc/gen: invalid storage driver %q", s)
}
