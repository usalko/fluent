...
type (
	...
	Config struct {
		...
		// holds the generated code. For example, "./project/fluent".
		//
		// By default, 'fluent generate ./fluent/schema' uses './fluent' as a
		// target directory.
		Target string

		// Package defines the Go package path of the target directory
		// mentioned above. For example, "github.com/org/project/fluent".
		//
		// By default, for schema package named "<project>/fluent/schema",
		// 'fluent generate' uses "<project>/fluent" as a default package.
		Package string
	...
	// Graph holds the nodes/fluentities of the loaded graph schema. Note that, it doesn't
...
func (g *Graph) Tables() (all []*schema.Table, err error) {
	...
	for _, n := range g.Nodes {
		...
		for _, idx := range n.Indexes {
			...
			// Set the fluent_sql.IndexAnnotation from the schema if exists.
...
// ModuleInfo returns the github.com/usalko/fluent version.
func (Config) ModuleInfo() (m debug.Module) {
	const pkg = "github.com/usalko/fluent"
	info, ok := debug.ReadBuildInfo()
	if !ok {
		return
	}
	// Was running as a CLI (ent/cmd/fluent).
	if info.Main.Path == pkg {
		return info.Main
	}
	// Or, as a main package (ent/flc).
...
func (p graphError) Error() string { return fmt.Sprintf("flc/gen: %s", p.msg) }
