...
import (
	...
	"github.com/usalko/fluent/flc/load"
...
func TestNewGraph(t *testing.T) {
	...
	_, err := NewGraph(&Config{Package: "flc/gen", Storage: drivers[0]}, T1)
	require.Error(err, "should fail due to missing types")

	graph, err := NewGraph(&Config{Package: "flc/gen", Storage: drivers[0]}, T1, T2)
	require.NoError(err)
...
func TestNewGraphRequiredLoop(t *testing.T) {
	_, err := NewGraph(&Config{Package: "flc/gen", Storage: drivers[0]}, &load.Schema{
		Name: "T1",
		Edges: []*load.Edge{
	...
	_, err = NewGraph(&Config{Package: "flc/gen", Storage: drivers[0]},
		&load.Schema{
			Name: "User",
			Edges: []*load.Edge{
...
func TestNewGraphDuplicateEdges(t *testing.T) {
	...
	require.EqualError(t, err, `flc/gen: User schema contains multiple "groups" edges`)
}
...
func TestNewGraphDuplicateEdgeField(t *testing.T) {
	...
	require.EqualError(t, err, `flc/gen: User schema cannot contain field and edge with the same name "parent"`)
}
...
func TestNewGraphThroughUndefinedType(t *testing.T) {
	...
	require.EqualError(t, err, `flc/gen: resolving edges: edge T1.groups defined with Through("groups_edge", T2.Type), but type T2 was not found`)
}
...
func TestNewGraphThroughInvalidRel(t *testing.T) {
	...
	require.EqualError(t, err, `flc/gen: resolving edges: edge T1.groups Through("groups_edge", T2.Type) is allowed only on M2M edges, but got: "O2O"`)
}
...
func TestNewGraphThroughDuplicates(t *testing.T) {
	...
	require.EqualError(t, err, `flc/gen: resolving edges: edge User.groups defined with Through("group_edges", T1.Type), but schema User already has an edge named group_edges`)
}
...
func TestRelation(t *testing.T) {
	...
	graph, err := NewGraph(&Config{Package: "flc/gen"}, T1, T2)
	require.NoError(err)
	require.NotNil(graph)
	require.Len(graph.Nodes, 2)
...
func TestFKColumns(t *testing.T) {
	...
	require := require.New(t)
	graph, err := NewGraph(&Config{Package: "flc/gen", Storage: drivers[0]}, user, &load.Schema{Name: "Pet"})
	require.NoError(err)
	t1 := graph.Nodes[0]
	...
	graph, err = NewGraph(&Config{Package: "flc/gen", Storage: drivers[0]}, user,
		&load.Schema{
			Name: "Pet",
			Edges: []*load.Edge{
...
func TestAbortDuplicateFK(t *testing.T) {
	...
	g, err := NewGraph(&Config{Package: "flc/gen", Storage: drivers[0]}, user, pet, car)
...
func TestEnsureCorrectFK(t *testing.T) {
	...
	_, err := NewGraph(&Config{Package: "flc/gen", Storage: drivers[0]}, user, pet)
	require.EqualError(t, err, `flc/gen: set "User" foreign-keys: column "owner_id" definition on edge "pets" should be replaced with Field("owner_id") on its reference "owner"`)
