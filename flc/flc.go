...
// Package flc provides an interface for interacting with
// flc (ent codegen) as a package rather than an executable.
package flc
...
import (
	...
	"github.com/usalko/fluent/flc/internal"
...
func LoadGraph(schemaPath string, cfg *gen.Config) (*gen.Graph, error) {
	...
	if cfg.Package == "" {
		...
		// before the schema package (`<project>/fluent/schema`).
...
// the base directory for codegen will be "<project>/fluent".
//
// If no storage driver provided by option, SQL driver will be used.
//
//	flc.Generate("./fluent/path", &gen.Config{
...
//		 fluent_gql.WithConfig("../gqlgen.yml"),
//		 fluent_gql.WithSchema("../schema.graphql"),
//	)
//	if err != nil {
...
//		),
//		flc.Dependency(
//			flc.DependencyName("DB"),
//			flc.DependencyType(&sql.DB{}),
//		)
//	}
