...
type (
	// A SchemaSpec holds a serializable version of an fluent.Schema
...
func (c *Config) Load() (*SchemaSpec, error) {
	spec, err := c.load()
	if err != nil {
		return nil, fmt.Errorf("flc/load: parse schema dir: %w", err)
	}
	if len(c.Names) == 0 {
		return nil, fmt.Errorf("flc/load: no schema found in: %s", c.Path)
	}
	var b bytes.Buffer
	...
	if err != nil {
		return nil, fmt.Errorf("flc/load: execute template: %w", err)
	}
	buf, err := format.Source(b.Bytes())
	if err != nil {
		return nil, fmt.Errorf("flc/load: format template: %w", err)
	}
	if err := os.MkdirAll(".flc", os.ModePerm); err != nil {
		return nil, err
	}
	target := fmt.Sprintf(".flc/%s.go", filename(spec.PkgPath))
	if err := os.WriteFile(target, buf, 0644); err != nil {
		return nil, fmt.Errorf("flc/load: write file %s: %w", target, err)
	}
	defer os.RemoveAll(".flc")
	...
	for _, line := range strings.Split(out, "\n") {
		...
		if err != nil {
			return nil, fmt.Errorf("flc/load: unmarshal schema %s: %w", line, err)
		}
...
func filename(pkg string) string {
	...
	return fmt.Sprintf("flc_%s_%d", name, time.Now().Unix())
}
...
func gorun(target string, buildFlags []string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("flc/load: %s", err)
	}
	return s, nil
}
