...
func (m *TaskMutation) SetStatus(task_status task.Status) {
	m.status = &task_status
}
...
func (m *TaskMutation) SetUUID(uuid_uuid uuid.UUID) {
	m.uuid = &uuid_uuid
}
...
func (m *TaskMutation) AddedIDs(name string) []fluent.Value {
	switch name {
	case task.EdgeTeams:
		ids := make([]fluent.Value, 0, len(m.teams))
		for id := range m.teams {
			ids = append(ids, id)
		}
		return ids
...
func (m *TaskMutation) RemovedIDs(name string) []fluent.Value {
	switch name {
	case task.EdgeTeams:
		ids := make([]fluent.Value, 0, len(m.removedteams))
		for id := range m.removedteams {
			ids = append(ids, id)
		}
		return ids
...
var _ fluent.Mutation = (*TeamMutation)(nil)
...
// return value indicates that this field was not set, or was not defined in the
// schema.
func (m *TeamMutation) Field(name string) (fluent.Value, bool) {
	switch name {
	case team.FieldName:
		return m.Name()
	}
...
// returned if the mutation operation is not UpdateOne, or the query to the
// database failed.
func (m *TeamMutation) OldField(ctx context.Context, name string) (fluent.Value, error) {
	switch name {
	case team.FieldName:
...
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *TeamMutation) SetField(name string, value fluent.Value) error {
	switch name {
	case team.FieldName:
		v, ok := value.(string)
		if !ok {
...
// with the given name. The second boolean return value indicates that this field
// was not set, or was not defined in the schema.
func (m *TeamMutation) AddedField(name string) (fluent.Value, bool) {
	return nil, false
}
...
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *TeamMutation) AddField(name string, value fluent.Value) error {
	switch name {
	}
...
// AddedIDs returns all IDs (to other nodes) that were added for the given edge
// name in this mutation.
func (m *TeamMutation) AddedIDs(name string) []fluent.Value {
	switch name {
	case team.EdgeTasks:
		ids := make([]fluent.Value, 0, len(m.tasks))
		for id := range m.tasks {
			ids = append(ids, id)
		}
		return ids
...
// RemovedIDs returns all IDs (to other nodes) that were removed for the edge with
// the given name in this mutation.
func (m *TeamMutation) RemovedIDs(name string) []fluent.Value {
	switch name {
	case team.EdgeTasks:
		ids := make([]fluent.Value, 0, len(m.removedtasks))
		for id := range m.removedtasks {
			ids = append(ids, id)
		}
		return ids
