...
func (User) Fields() []fluent.Field {
	return []fluent.Field{
		...
		field.Strings("roles").
			Optional().
			Annotations(fluent_sql.Default(`[]`)),
		field.String("default_expr").
			Optional().
			Annotations(fluent_sql.DefaultExpr("lower('hello')")),
		field.String("default_exprs").
			Optional().
			Annotations(fluent_sql.DefaultExprs(map[string]string{
...
// Blog schema.
type Blog struct{ fluent.Schema }

func (Blog) Fields() []fluent.Field {
	return []fluent.Field{
		field.Int("id").
