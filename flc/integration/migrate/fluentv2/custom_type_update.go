...
func (ctu *CustomTypeUpdate) sqlSave(ctx context.Context) (n int, err error) {
	_spec := sqlgraph.NewUpdateSpec(custom_type.Table, custom_type.Columns, sqlgraph.NewFieldSpec(custom_type.FieldID, field.TypeInt))
	...
	if value, ok := ctu.mutation.Custom(); ok {
		_spec.SetField(custom_type.FieldCustom, field.TypeString, value)
	}
	if ctu.mutation.CustomCleared() {
		_spec.ClearField(custom_type.FieldCustom, field.TypeString)
	}
	if value, ok := ctu.mutation.Tz0(); ok {
		_spec.SetField(custom_type.FieldTz0, field.TypeTime, value)
	}
	if ctu.mutation.Tz0Cleared() {
		_spec.ClearField(custom_type.FieldTz0, field.TypeTime)
	}
	if value, ok := ctu.mutation.Tz3(); ok {
		_spec.SetField(custom_type.FieldTz3, field.TypeTime, value)
	}
	if ctu.mutation.Tz3Cleared() {
		_spec.ClearField(custom_type.FieldTz3, field.TypeTime)
	}
...
func (ctuo *CustomTypeUpdateOne) sqlSave(ctx context.Context) (_node *CustomType, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`fluentv2: missing "CustomType.id" for update`)}
	}
