...
func (gic *GroupInfoCreate) gremlin() *dsl.Traversal {
	...
	v := g.AddV(group_info.Label)
	if value, ok := gic.mutation.Desc(); ok {
		v.Property(dsl.Single, group_info.FieldDesc, value)
	}
	if value, ok := gic.mutation.MaxUsers(); ok {
		v.Property(dsl.Single, group_info.FieldMaxUsers, value)
	}
	for _, id := range gic.mutation.GroupsIDs() {
		...
		constraints = append(constraints, &constraint{
			...
			test: __.Is(p.NEQ(0)).Constant(NewErrUniqueEdge(group_info.Label, group.InfoLabel, id)),
		})
	}
