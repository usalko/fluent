...
func (evsu *ExValueScanUpdate) gremlin() *dsl.Traversal {
	...
	if evsu.mutation.BinaryOptionalCleared() {
		properties = append(properties, ex_value_scan.FieldBinaryOptional)
	}
	if evsu.mutation.TextOptionalCleared() {
		properties = append(properties, ex_value_scan.FieldTextOptional)
	}
	if evsu.mutation.CustomOptionalCleared() {
		properties = append(properties, ex_value_scan.FieldCustomOptional)
	}
	if len(properties) > 0 {
