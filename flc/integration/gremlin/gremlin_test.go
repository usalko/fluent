...
func TestGremlin(t *testing.T) {
	client, err := fluent.Open("gremlin", "http://localhost:8182")
...
var tests = []func(*testing.T, *fluent.Client){
	Tx,
	Types,
	Clone,
	Sanity,
...
func O2OTwoTypes(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
...
func O2OSameType(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
	...
	values, err := client.Node.Query().
		...
		Order(fluent.Asc(node.FieldValue)).
...
func O2OSelfRef(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
...
func O2MTwoTypes(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
...
func O2MSameType(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
...
//	// count will be 2, even though we've created only one relation above.
func M2MSelfRef(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
...
func M2MSameType(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
...
func M2MTwoTypes(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
	...
	require.Equal([]string{bar.Name, foo.Name}, hub.QueryUsers().Order(fluent.Asc(user.FieldName)).GroupBy(user.FieldName).StringsX(ctx))
	require.Equal([]string{hub.Name, lab.Name}, bar.QueryGroups().Order(fluent.Asc(user.FieldName)).GroupBy(user.FieldName).StringsX(ctx))
...
func Types(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	require := require.New(t)
