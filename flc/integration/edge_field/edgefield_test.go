...
import (
	...
	"github.com/usalko/fluent/flc/integration/edge_field/fluent"
	...
	"github.com/usalko/fluent/flc/integration/edge_field/fluent/pet"
...
func TestEdgeField(t *testing.T) {
	...
	ps1 := client.Post.Create().SetText("github.com/usalko/fluent").SaveX(ctx)
	...
	require.True(t, fluent.IsConstraintError(err), "UNIQUE constraint failed: metadata.id")
	...
	require.Error(t, err)
	require.True(t, fluent.IsConstraintError(err))
...
func TestNamedEdges(t *testing.T) {
	client, err := fluent.Open(dialect.SQLite, "file:ent?mode=memory&_fk=1")
	require.NoError(t, err)
	defer client.Close()
	ctx := context.Background()
	require.NoError(t, client.Schema.Create(ctx, migrate.WithGlobalUniqueID(true)))
	...
	u1 = client.User.Query().
		...
		WithNamedPets("Named", func(q *fluent.PetQuery) {
			q.Select(pet.FieldID)
		}).
		OnlyX(ctx)
