...
func TestMySQL(t *testing.T) {
	for version, port := range map[string]int{"56": 3306, "57": 3307, "8": 3308} {
		t.Run(version, func(t *testing.T) {
			...
			client, err := generated_code.Open("mysql", fmt.Sprintf("root:pass@tcp(localhost:%d)/json", port))
...
func TestPostgres(t *testing.T) {
	for version, port := range map[string]int{"10": 5430, "11": 5431, "12": 5433, "13": 5434} {
		t.Run(version, func(t *testing.T) {
			...
			client, err := generated_code.Open(dialect.Postgres, dsn+" dbname=json")
			require.NoError(t, err, "connecting to json database")
			defer client.Close()
			err = client.Schema.Create(context.Background(), migrate.WithGlobalUniqueID(true))
			require.NoError(t, err)
...
func TestSQLite(t *testing.T) {
	client, err := generated_code.Open("sqlite3", "file:ent?mode=memory&cache=shared&_fk=1")
	require.NoError(t, err)
	defer client.Close()
	ctx := context.Background()
...
func Ints(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	ints := []int{1, 2, 3}
...
func IntsValidate(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	xs := []int{1, 2, 3}
...
func Floats(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	flts := []float64{1, 2, 3}
...
func FloatsValidate(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	xs := []float64{1, 2, 3}
...
func Strings(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	str := []string{"a", "b", "c"}
...
func StringsValidate(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	xs := []string{"a", "b", "c"}
...
func Any(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
...
func RawMessage(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	raw := json.RawMessage("{}")
...
func NetAddr(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	...
	f, ok := reflect.TypeOf(generated_code.User{}).FieldByName("Addr")
	require.True(t, ok)
	require.Equal(t, "-", f.Tag.Get("json"))
}

func Dirs(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
...
func URL(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	usr := client.User.Create().SaveX(ctx)
...
func URLs(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	u1, err := url.Parse("https://github.com/a8m")
	require.NoError(t, err)
	u2, err := url.Parse("https://github.com/fluent")
	require.NoError(t, err)
...
func Predicates(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()

	client.User.Delete().ExecX(ctx)
	u1, err := url.Parse("https://github.com/a8m/fluent")
	require.NoError(t, err)
	u2, err := url.Parse("ftp://a8m@github.com/fluent")
	require.NoError(t, err)
...
func Order(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	client.User.Delete().ExecX(ctx)
	client.User.CreateBulk(
...
func Scan(t *testing.T, client *generated_code.Client) {
	ctx := context.Background()
	all := client.User.Query().Order(generated_code.Asc(user.FieldID)).AllX(ctx)
	require.NotEmpty(t, all)
	var scanned []*generated_code.User
	// Select all non-sensitive fields.
	client.User.Query().Order(generated_code.Asc(user.FieldID)).Select(user.Columns[:len(user.Columns)-2]...).ScanX(ctx, &scanned)
