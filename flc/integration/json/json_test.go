...
import (
	...
	"github.com/usalko/fluent/flc/integration/json/fluent/migrate"
	"github.com/usalko/fluent/flc/integration/json/fluent/schema"
	"github.com/usalko/fluent/flc/integration/json/fluent/user"
...
func TestMySQL(t *testing.T) {
	for version, port := range map[string]int{"56": 3306, "57": 3307, "8": 3308} {
		t.Run(version, func(t *testing.T) {
			...
			client, err := fluent.Open("mysql", fmt.Sprintf("root:pass@tcp(localhost:%d)/json", port))
...
func TestPostgres(t *testing.T) {
	for version, port := range map[string]int{"10": 5430, "11": 5431, "12": 5433, "13": 5434} {
		t.Run(version, func(t *testing.T) {
			...
			client, err := fluent.Open(dialect.Postgres, dsn+" dbname=json")
			require.NoError(t, err, "connecting to json database")
			defer client.Close()
			err = client.Schema.Create(context.Background(), migrate.WithGlobalUniqueID(true))
			require.NoError(t, err)
...
func Ints(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	ints := []int{1, 2, 3}
...
func IntsValidate(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	xs := []int{1, 2, 3}
...
func Floats(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	flts := []float64{1, 2, 3}
...
func FloatsValidate(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	xs := []float64{1, 2, 3}
...
func Strings(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	str := []string{"a", "b", "c"}
...
func StringsValidate(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	xs := []string{"a", "b", "c"}
...
func Any(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
...
func RawMessage(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	raw := json.RawMessage("{}")
...
func NetAddr(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	...
	f, ok := reflect.TypeOf(fluent.User{}).FieldByName("Addr")
	require.True(t, ok)
	require.Equal(t, "-", f.Tag.Get("json"))
}

func Dirs(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
...
func URL(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	usr := client.User.Create().SaveX(ctx)
...
func URLs(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
...
func Predicates(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
...
func Order(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	client.User.Delete().ExecX(ctx)
	client.User.CreateBulk(
...
func Scan(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	all := client.User.Query().Order(fluent.Asc(user.FieldID)).AllX(ctx)
	require.NotEmpty(t, all)
	var scanned []*fluent.User
	// Select all non-sensitive fields.
	client.User.Query().Order(fluent.Asc(user.FieldID)).Select(user.Columns[:len(user.Columns)-2]...).ScanX(ctx, &scanned)
