...
import (
	...
	fluent "github.com/usalko/fluent/flc/integration/edge_schema/fluent"
	...
	"github.com/usalko/fluent/flc/integration/edge_schema/fluent/migrate"
	...
	_ "github.com/usalko/fluent/flc/integration/edge_schema/fluent/runtime"
...
func TestEdgeSchemaWithID(t *testing.T) {
	...
	require.True(t, fluent.IsConstraintError(err), "duplicate edge error, because edge exists with a different 'joined_at' value")
	...
	require.Equal(t, lab.ID, a8m.QueryJoinedGroups().QueryGroup().Order(fluent.Desc(group.FieldID)).FirstIDX(ctx))
	...
	require.True(t, fluent.IsConstraintError(err), "unique constraint failed: user_groups.user_id, user_groups.group_id")

	users := client.User.Query().WithJoinedGroups(func(q *fluent.UserGroupQuery) { q.WithGroup() }).AllX(ctx)
...
func TestEdgeSchemaCompositeID(t *testing.T) {
	...
	for _, k := range []*fluent.TweetLike{
	...
	client.TweetLike.Query().GroupBy(tweetlike.FieldUserID).Aggregate(fluent.Count()).ScanX(ctx, &v)
...
func TestEdgeSchemaBidiWithID(t *testing.T) {
	...
	for _, f1 := range []*fluent.Friendship{
...
func TestEdgeSchemaBidiCompositeID(t *testing.T) {
	...
	require.True(t, fluent.IsConstraintError(err), "duplicate edge error, because edge may contain a different 'weight' value in the database")
	...
	client.Relationship.Query().GroupBy(relationship.FieldUserID).Aggregate(fluent.Count()).ScanX(ctx, &v)
	require.EqualValues(
		t,
...
func TestEdgeSchemaForO2M(t *testing.T) {
	...
	require.True(t, fluent.IsConstraintError(err), "Tweet can have only one author")
...
func TestEdgeSchemaEntQL(t *testing.T) {
	...
	q2.Filter().Where(fluent_ql.Not(fluent_ql.HasEdge("likes")))
	...
	q2.Filter().Where(fluent_ql.HasEdgeWith("likes", fluent_ql.FieldEQ(tweetlike.FieldTweetID, tweets[0].ID)))
	require.Equal(t, 2, q1.CountX(ctx))
	require.Equal(t, 2, q2.CountX(ctx))
	q1, q2 = client.User.Query(), client.User.Query()
	q1.Filter().WhereHasLikesWith(tweetlike.TweetID(tweets[1].ID))
	q2.Filter().Where(fluent_ql.HasEdgeWith("likes", fluent_ql.FieldEQ(tweetlike.FieldTweetID, tweets[1].ID)))
...
func TestEdgeSchemaTypeMatching(t *testing.T) {
	...
	// Assoc query.
	require.Equal(t, []string{"a", "b"}, proc.QueryFiles().Order(fluent.Asc(file.FieldName)).Select(file.FieldName).StringsX(ctx))
	require.Equal(t, []int{files[0].ID, files[1].ID}, proc.QueryAttachedFiles().Order(fluent.Asc(attachedfile.FieldFID)).Select(attachedfile.FieldFID).IntsX(ctx))
