...
func (ttu *TweetTagUpdate) check() error {
	if ttu.mutation.TagCleared() && len(ttu.mutation.TagIDs()) > 0 {
		return errors.New(`fluent: clearing a required unique edge "TweetTag.tag"`)
	}
	if ttu.mutation.TweetCleared() && len(ttu.mutation.TweetIDs()) > 0 {
		return errors.New(`fluent: clearing a required unique edge "TweetTag.tweet"`)
	}
	return nil
}
...
func (ttu *TweetTagUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(tweet_tag.Table, tweet_tag.Columns, sqlgraph.NewFieldSpec(tweet_tag.FieldID, field.TypeUUID))
	...
	if value, ok := ttu.mutation.AddedAt(); ok {
		_spec.SetField(tweet_tag.FieldAddedAt, field.TypeTime, value)
	}
...
func (ttuo *TweetTagUpdateOne) sqlSave(ctx context.Context) (_node *TweetTag, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`fluent: missing "TweetTag.id" for update`)}
	}
