...
func (tlq *TweetLikeQuery) Order(o ...tweet_like.OrderOption) *TweetLikeQuery {
...
func (tlq *TweetLikeQuery) QueryTweet() *TweetQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(tweet_like.Table, tweet_like.TweetColumn, selector),
			sqlgraph.To(tweet.Table, tweet.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, false, tweet_like.TweetTable, tweet_like.TweetColumn),
		)
...
func (tlq *TweetLikeQuery) QueryUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(tweet_like.Table, tweet_like.UserColumn, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, false, tweet_like.UserTable, tweet_like.UserColumn),
		)
		fromU = sqlgraph.SetNeighbors(tlq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (tlq *TweetLikeQuery) First(ctx context.Context) (*TweetLike, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{tweet_like.Label}
	}
	return nodes[0], nil
}
...
func (tlq *TweetLikeQuery) Only(ctx context.Context) (*TweetLike, error) {
	...
	case 0:
		return nil, &NotFoundError{tweet_like.Label}
	default:
		return nil, &NotSingularError{tweet_like.Label}
	}
}
...
func (tlq *TweetLikeQuery) Clone() *TweetLikeQuery {
	...
	return &TweetLikeQuery{
		config:     tlq.config,
		ctx:        tlq.ctx.Clone(),
		order:      append([]tweet_like.OrderOption{}, tlq.order...),
...
//		GroupBy(tweet_like.FieldLikedAt).
...
func (tlq *TweetLikeQuery) GroupBy(field string, fields ...string) *TweetLikeGroupBy {
	...
	grbuild.label = tweet_like.Label
...
//
//	client.TweetLike.Query().
//		Select(tweet_like.FieldLikedAt).
//		Scan(ctx, &v)
...
func (tlq *TweetLikeQuery) prepareQuery(ctx context.Context) error {
	...
	if tweet_like.Policy == nil {
		return errors.New("fluent: uninitialized tweet_like.Policy (forgotten import fluent/runtime?)")
	}
	if err := tweet_like.Policy.EvalQuery(ctx, tlq); err != nil {
		return err
	}
	return nil
}
...
func (tlq *TweetLikeQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(tweet_like.Table, tweet_like.Columns, nil)
	_spec.From = tlq.sql
	...
	if fields := tlq.ctx.Fields; len(fields) > 0 {
		...
		if tlq.withTweet != nil {
			_spec.Node.AddColumnOnce(tweet_like.FieldTweetID)
		}
		if tlq.withUser != nil {
			_spec.Node.AddColumnOnce(tweet_like.FieldUserID)
		}
	}
...
func (tlq *TweetLikeQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1 := builder.Table(tweet_like.Table)
