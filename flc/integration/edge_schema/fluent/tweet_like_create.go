...
func (tlc *TweetLikeCreate) defaults() error {
	if _, ok := tlc.mutation.LikedAt(); !ok {
		if tweet_like.DefaultLikedAt == nil {
			return fmt.Errorf("fluent: uninitialized tweet_like.DefaultLikedAt (forgotten import fluent/runtime?)")
		}
		v := tweet_like.DefaultLikedAt()
...
func (tlc *TweetLikeCreate) createSpec() (*TweetLike, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(tweet_like.Table, nil)
	)
	_spec.OnConflict = tlc.conflict
	if value, ok := tlc.mutation.LikedAt(); ok {
		_spec.SetField(tweet_like.FieldLikedAt, field.TypeTime, value)
		_node.LikedAt = value
	}
...
//		// update values.
//		Update(func(u *fluent.TweetLikeUpsert) {
//			SetLikedAt(v+v).
//		}).
//		Exec(ctx)
...
func (u *TweetLikeUpsert) SetLikedAt(v time.Time) *TweetLikeUpsert {
	u.Set(tweet_like.FieldLikedAt, v)
	return u
}
...
func (u *TweetLikeUpsert) UpdateTweetID() *TweetLikeUpsert {
	u.SetExcluded(tweet_like.FieldTweetID)
	return u
}
