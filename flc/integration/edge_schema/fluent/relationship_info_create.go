...
func (ric *RelationshipInfoCreate) createSpec() (*RelationshipInfo, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(relationship_info.Table, sqlgraph.NewFieldSpec(relationship_info.FieldID, field.TypeInt))
	)
	_spec.OnConflict = ric.conflict
	if value, ok := ric.mutation.Text(); ok {
		_spec.SetField(relationship_info.FieldText, field.TypeString, value)
		_node.Text = value
	}
	return _node, _spec
}
...
//		// update values.
//		Update(func(u *fluent.RelationshipInfoUpsert) {
//			SetText(v+v).
//		}).
//		Exec(ctx)
...
func (u *RelationshipInfoUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("fluent: missing options for RelationshipInfoCreate.OnConflict")
	}
...
func (u *RelationshipInfoUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("fluent: OnConflict was set for builder %d. Set it on the RelationshipInfoCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("fluent: missing options for RelationshipInfoCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
