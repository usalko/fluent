...
func (utq *UserTweetQuery) Order(o ...user_tweet.OrderOption) *UserTweetQuery {
...
func (utq *UserTweetQuery) QueryUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(user_tweet.Table, user_tweet.FieldID, selector),
...
func (utq *UserTweetQuery) First(ctx context.Context) (*UserTweet, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{user_tweet.Label}
	}
	return nodes[0], nil
}
...
func (utq *UserTweetQuery) Only(ctx context.Context) (*UserTweet, error) {
	...
	case 0:
		return nil, &NotFoundError{user_tweet.Label}
	default:
		return nil, &NotSingularError{user_tweet.Label}
	}
}
...
func (utq *UserTweetQuery) OnlyID(ctx context.Context) (id int, err error) {
	...
	case 0:
		err = &NotFoundError{user_tweet.Label}
	default:
		err = &NotSingularError{user_tweet.Label}
	}
	return
}
...
func (utq *UserTweetQuery) IDs(ctx context.Context) (ids []int, err error) {
	...
	if err = utq.Select(user_tweet.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (utq *UserTweetQuery) Clone() *UserTweetQuery {
	...
	return &UserTweetQuery{
		config:     utq.config,
		ctx:        utq.ctx.Clone(),
		order:      append([]user_tweet.OrderOption{}, utq.order...),
...
//		GroupBy(user_tweet.FieldCreatedAt).
...
func (utq *UserTweetQuery) GroupBy(field string, fields ...string) *UserTweetGroupBy {
	...
	grbuild.label = user_tweet.Label
...
//
//	client.UserTweet.Query().
//		Select(user_tweet.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (utq *UserTweetQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(user_tweet.Table, user_tweet.Columns, sqlgraph.NewFieldSpec(user_tweet.FieldID, field.TypeInt))
	_spec.From = utq.sql
	...
	if fields := utq.ctx.Fields; len(fields) > 0 {
		...
		for i := range fields {
			if fields[i] != user_tweet.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, fields[i])
			}
		}
		if utq.withUser != nil {
			_spec.Node.AddColumnOnce(user_tweet.FieldUserID)
		}
		if utq.withTweet != nil {
			_spec.Node.AddColumnOnce(user_tweet.FieldTweetID)
		}
	}
...
func (utq *UserTweetQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1 := builder.Table(user_tweet.Table)
