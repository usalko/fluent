...
func (gtc *GroupTagCreate) createSpec() (*GroupTag, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(group_tag.Table, sqlgraph.NewFieldSpec(group_tag.FieldID, field.TypeInt))
	)
...
//		// update values.
//		Update(func(u *fluent.GroupTagUpsert) {
//			SetTagID(v+v).
//		}).
//		Exec(ctx)
...
func (u *GroupTagUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("fluent: missing options for GroupTagCreate.OnConflict")
	}
...
func (u *GroupTagUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("fluent: OnConflict was set for builder %d. Set it on the GroupTagCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("fluent: missing options for GroupTagCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
