...
import (
	...
	"github.com/usalko/fluent/flc/integration/config/fluent"
	"github.com/usalko/fluent/flc/integration/config/fluent/migrate"
	"github.com/usalko/fluent/flc/integration/config/fluent/schema"
...
func TestSchemaConfig(t *testing.T) {
	...
	idIncremental := schema.User{}.Fields()[0].Descriptor().Annotations[0].(fluent_sql.Annotation).Incremental
	require.Equal(t, *idIncremental, migrate.Tables[0].Columns[0].Increment)
	size := schema.User{}.Fields()[1].Descriptor().Annotations[0].(fluent_sql.Annotation).Size
