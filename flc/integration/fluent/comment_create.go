...
func (cc *CommentCreate) SetDir(schemadir_dir schemadir.Dir) *CommentCreate {
	cc.mutation.SetDir(schemadir_dir)
	return cc
}

// SetNillableDir sets the "dir" field if the given value is not nil.
func (cc *CommentCreate) SetNillableDir(schemadir_dir *schemadir.Dir) *CommentCreate {
	if schemadir_dir != nil {
		cc.SetDir(*schemadir_dir)
	}
	return cc
}
...
func (cc *CommentCreate) check() error {
	if _, ok := cc.mutation.UniqueInt(); !ok {
		return &ValidationError{Name: "unique_int", err: errors.New(`fluent: missing required field "Comment.unique_int"`)}
	}
	if _, ok := cc.mutation.UniqueFloat(); !ok {
		return &ValidationError{Name: "unique_float", err: errors.New(`fluent: missing required field "Comment.unique_float"`)}
	}
	return nil
}
...
//		// update values.
//		Update(func(u *fluent.CommentUpsert) {
//			SetUniqueInt(v+v).
//		}).
//		Exec(ctx)
