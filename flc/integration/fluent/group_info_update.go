...
func (giu *GroupInfoUpdate) sqlSave(ctx context.Context) (n int, err error) {
	_spec := sqlgraph.NewUpdateSpec(group_info.Table, group_info.Columns, sqlgraph.NewFieldSpec(group_info.FieldID, field.TypeInt))
	...
	if value, ok := giu.mutation.AddedMaxUsers(); ok {
		_spec.AddField(group_info.FieldMaxUsers, field.TypeInt, value)
	}
