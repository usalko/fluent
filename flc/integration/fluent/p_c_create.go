...
func (pc *PCCreate) sqlSave(ctx context.Context) (*PC, error) {
...
func (pc *PCCreate) createSpec() (*PC, *sqlgraph.CreateSpec) {
	var (
		_node = &PC{config: pc.config}
		_spec = sqlgraph.NewCreateSpec(p_c.Table, sqlgraph.NewFieldSpec(p_c.FieldID, field.TypeInt))
	)
...
//		).
//		Exec(ctx)
func (pc *PCCreate) OnConflict(opts ...sql.ConflictOption) *PCUpsertOne {
	pc.conflict = opts
	return &PCUpsertOne{
		create: pc,
	}
...
//		Exec(ctx)
func (pc *PCCreate) OnConflictColumns(columns ...string) *PCUpsertOne {
