...
func (evsu *ExValueScanUpdate) sqlSave(ctx context.Context) (n int, err error) {
	_spec := sqlgraph.NewUpdateSpec(ex_value_scan.Table, ex_value_scan.Columns, sqlgraph.NewFieldSpec(ex_value_scan.FieldID, field.TypeInt))
	...
	if evsu.mutation.BinaryOptionalCleared() {
		_spec.ClearField(ex_value_scan.FieldBinaryOptional, field.TypeString)
	}
	...
	if evsu.mutation.TextOptionalCleared() {
		_spec.ClearField(ex_value_scan.FieldTextOptional, field.TypeString)
	}
	...
	if evsu.mutation.CustomOptionalCleared() {
		_spec.ClearField(ex_value_scan.FieldCustomOptional, field.TypeString)
	}
	_spec.AddModifiers(evsu.modifiers...)
	if n, err = sqlgraph.UpdateNodes(ctx, evsu.driver, _spec); err != nil {
		if _, ok := err.(*sqlgraph.NotFoundError); ok {
			err = &NotFoundError{ex_value_scan.Label}
...
func (evsuo *ExValueScanUpdateOne) sqlSave(ctx context.Context) (_node *ExValueScan, err error) {
	...
	if fields := evsuo.fields; len(fields) > 0 {
		...
		_spec.Node.Columns = append(_spec.Node.Columns, ex_value_scan.FieldID)
		for _, f := range fields {
			if !ex_value_scan.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("fluent: invalid field %q for query", f)}
			}
			if f != ex_value_scan.FieldID {
