...
type FileType struct {
	...
	Type file_type.Type `json:"type,omitempty"`
	// State holds the value of the "state" field.
	State file_type.State `json:"state,omitempty"`
...
func (*FileType) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case file_type.FieldID:
			values[i] = new(sql.NullInt64)
		case file_type.FieldName, file_type.FieldType, file_type.FieldState:
	...
	for i := range columns {
		...
		case file_type.FieldType:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				ft.Type = file_type.Type(value.String)
			}
		case file_type.FieldState:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field state", values[i])
			} else if value.Valid {
				ft.State = file_type.State(value.String)
			}
		default:
...
// Value returns the fluent.Value that was dynamically selected and assigned to the FileType.
// This includes values selected through modifiers, order, etc.
func (ft *FileType) Value(name string) (fluent.Value, error) {
