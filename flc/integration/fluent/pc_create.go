...
func (pb *PCCreateBulk) Save(ctx context.Context) ([]*PC, error) {
	if pb.err != nil {
		return nil, pb.err
	}
	specs := make([]*sqlgraph.CreateSpec, len(pb.builders))
	nodes := make([]*PC, len(pb.builders))
	mutators := make([]Mutator, len(pb.builders))
	for i := range pb.builders {
		func(i int, root context.Context) {
			builder := pb.builders[i]
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				...
				if i < len(mutators)-1 {
					_, err = mutators[i+1].Mutate(root, pb.builders[i+1].mutation)
				} else {
					spec := &sqlgraph.BatchCreateSpec{Nodes: specs}
					spec.OnConflict = pb.conflict
					// Invoke the actual operation on the latest mutation in the chain.
					if err = sqlgraph.BatchCreate(ctx, pb.driver, spec); err != nil {
						if sqlgraph.IsConstraintError(err) {
							err = &ConstraintError{msg: err.Error(), wrap: err}
						}
					}
	...
	if len(mutators) > 0 {
		if _, err := mutators[0].Mutate(ctx, pb.builders[0].mutation); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pb *PCCreateBulk) SaveX(ctx context.Context) []*PC {
	v, err := pb.Save(ctx)
	if err != nil {
		panic(err)
	}
...
// Exec executes the query.
func (pb *PCCreateBulk) Exec(ctx context.Context) error {
	_, err := pb.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (pb *PCCreateBulk) ExecX(ctx context.Context) {
...
//		).
//		Exec(ctx)
func (pb *PCCreateBulk) OnConflict(opts ...sql.ConflictOption) *PCUpsertBulk {
	pb.conflict = opts
	return &PCUpsertBulk{
		create: pb,
	}
...
//		OnConflict(sql.ConflictColumns(columns...)).
//		Exec(ctx)
func (pb *PCCreateBulk) OnConflictColumns(columns ...string) *PCUpsertBulk {
	pb.conflict = append(pb.conflict, sql.ConflictColumns(columns...))
	return &PCUpsertBulk{
		create: pb,
	}
}
