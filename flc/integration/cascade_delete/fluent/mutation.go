...
import (
	...
	"github.com/usalko/fluent/flc/integration/cascade_delete/fluent/predicate"
...
func (m *PostMutation) AddedIDs(name string) []fluent.Value {
	...
	case post.EdgeComments:
		ids := make([]fluent.Value, 0, len(m.comments))
		for id := range m.comments {
			ids = append(ids, id)
		}
		return ids
...
func (m *PostMutation) RemovedIDs(name string) []fluent.Value {
	switch name {
	case post.EdgeComments:
		ids := make([]fluent.Value, 0, len(m.removedcomments))
		for id := range m.removedcomments {
			ids = append(ids, id)
		}
		return ids
...
func (m *UserMutation) AddedIDs(name string) []fluent.Value {
	switch name {
	case user.EdgePosts:
		ids := make([]fluent.Value, 0, len(m.posts))
		for id := range m.posts {
			ids = append(ids, id)
		}
		return ids
...
func (m *UserMutation) RemovedIDs(name string) []fluent.Value {
	switch name {
	case user.EdgePosts:
		ids := make([]fluent.Value, 0, len(m.removedposts))
		for id := range m.removedposts {
			ids = append(ids, id)
		}
		return ids
