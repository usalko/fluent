...
import (
	...
	"github.com/usalko/fluent/flc/integration/template/fluent"
	"github.com/usalko/fluent/flc/integration/template/fluent/hook"
	"github.com/usalko/fluent/flc/integration/template/fluent/migrate"
	"github.com/usalko/fluent/flc/integration/template/fluent/pet"
	"github.com/usalko/fluent/flc/integration/template/fluent/user"
...
func TestCustomTemplate(t *testing.T) {
	...
	require.Equal(t, &fluent.Field{Type: "int", Name: "Age", Value: "1"}, node.Fields[0])
	require.Equal(t, &fluent.Edge{Type: "User", Name: "Owner", IDs: []int{u.ID}}, node.Edges[0])

	node, err = client.Node(ctx, u.ID)
	require.NoError(t, err)
	require.Equal(t, u.ID, node.ID)
	require.Equal(t, &fluent.Field{Type: "string", Name: "Name", Value: "\"a8m\""}, node.Fields[0])
	require.Equal(t, &fluent.Edge{Type: "Pet", Name: "Pets", IDs: []int{p.ID}}, node.Edges[0])

	node, err = client.Node(ctx, g.ID)
	require.NoError(t, err)
	require.Equal(t, g.ID, node.ID)
	require.Equal(t, &fluent.Field{Type: "int", Name: "MaxUsers", Value: "10"}, node.Fields[0])
