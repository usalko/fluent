...
import (
	...
	fluent "github.com/usalko/fluent/flc/integration/custom_id/fluent"
...
func TestMySQL(t *testing.T) {
	for version, port := range map[string]int{"56": 3306, "57": 3307, "8": 3308} {
		...
		t.Run(version, func(t *testing.T) {
			...
			client, err := fluent.Open("mysql", cfg.FormatDSN())
...
func TestPostgres(t *testing.T) {
	for version, port := range map[string]int{"10": 5430, "11": 5431, "12": 5433, "13": 5434} {
		t.Run(version, func(t *testing.T) {
			...
			client := fluent.NewClient(fluent.Driver(fluent_sql.OpenDB(dialect.Postgres, db)))
...
func CustomID(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	nat := client.User.Create().SaveX(ctx)
	require.Equal(t, 1, nat.ID)
	_, err := client.User.Create().SetID(1).Save(ctx)
	require.True(t, fluent.IsConstraintError(err), "duplicate id")
	...
	require.Equal(t, []int{1, 5}, hub.QueryUsers().Order(fluent.Asc(user.FieldID)).IDsX(ctx))
	...
	pets := client.Pet.Query().WithFriends().WithBestFriend().Order(fluent.Asc(pet.FieldID)).AllX(ctx)
	...
	require.Equal(t, []string{"pedro", "xabi"}, pets[2].QueryFriends().Order(fluent.Asc(pet.FieldID)).IDsX(ctx))
	...
	t.Run("Upsert", func(t *testing.T) {
		...
		client.Blob.Create().
			SetID(id).
			OnConflictColumns(blob.FieldID).
			Update(func(set *fluent.BlobUpsert) {
				set.AddCount(1)
			}).
			ExecX(ctx)
...
func BytesID(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
