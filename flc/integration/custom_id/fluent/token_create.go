...
func (tc *TokenCreate) SetID(sid_id sid.ID) *TokenCreate {
	tc.mutation.SetID(sid_id)
	return tc
}

// SetNillableID sets the "id" field if the given value is not nil.
func (tc *TokenCreate) SetNillableID(sid_id *sid.ID) *TokenCreate {
	if sid_id != nil {
		tc.SetID(*sid_id)
	}
	return tc
...
//		// update values.
//		Update(func(u *fluent.TokenUpsert) {
//			SetBody(v+v).
//		}).
//		Exec(ctx)
...
func (u *TokenUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("fluent: missing options for TokenCreate.OnConflict")
	}
...
func (u *TokenUpsertOne) ID(ctx context.Context) (id sid.ID, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("fluent: TokenUpsertOne.ID is not supported by MySQL driver. Use TokenUpsertOne.Exec instead")
	}
...
func (u *TokenUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("fluent: OnConflict was set for builder %d. Set it on the TokenCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("fluent: missing options for TokenCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
