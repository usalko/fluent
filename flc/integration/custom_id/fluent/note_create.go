...
func (nc *NoteCreate) SetID(schema_noteid schema.NoteID) *NoteCreate {
	nc.mutation.SetID(schema_noteid)
	return nc
}

// SetNillableID sets the "id" field if the given value is not nil.
func (nc *NoteCreate) SetNillableID(schema_noteid *schema.NoteID) *NoteCreate {
	if schema_noteid != nil {
		nc.SetID(*schema_noteid)
	}
	return nc
}
...
func (nc *NoteCreate) check() error {
	if v, ok := nc.mutation.ID(); ok {
		if err := note.IDValidator(string(v)); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`fluent: validator failed for field "Note.id": %w`, err)}
		}
	}
	return nil
}
...
//		// update values.
//		Update(func(u *fluent.NoteUpsert) {
//			SetText(v+v).
//		}).
//		Exec(ctx)
...
func (u *NoteUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("fluent: missing options for NoteCreate.OnConflict")
	}
...
func (u *NoteUpsertOne) ID(ctx context.Context) (id schema.NoteID, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("fluent: NoteUpsertOne.ID is not supported by MySQL driver. Use NoteUpsertOne.Exec instead")
	}
...
func (u *NoteUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("fluent: OnConflict was set for builder %d. Set it on the NoteCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("fluent: missing options for NoteCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
