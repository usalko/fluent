...
import (
	...
	sqlschema "github.com/usalko/fluent/dialect/sql/schema"
...
func TestSQLite(t *testing.T) {
	t.Parallel()
	client := fluent_test.Open(t, dialect.SQLite, "file:ent?mode=memory&cache=shared&_fk=1", opts)
	defer client.Close()
...
func TestMySQL(t *testing.T) {
	for version, port := range map[string]int{"56": 3306, "57": 3307, "8": 3308} {
		...
		t.Run(version, func(t *testing.T) {
			t.Parallel()
			client := fluent_test.Open(t, dialect.MySQL, fmt.Sprintf("root:pass@tcp(%s)/test?parseTime=True", addr), opts)
			defer client.Close()
			for _, tt := range tests {
				name := runtime.FuncForPC(reflect.ValueOf(tt).Pointer()).Name()
				t.Run(name[strings.LastIndex(name, ".")+1:], func(t *testing.T) {
...
func TestPostgres(t *testing.T) {
	for version, port := range map[string]int{"10": 5430, "11": 5431, "12": 5432, "13": 5433, "14": 5434, "15": 5435} {
		...
		t.Run(version, func(t *testing.T) {
			t.Parallel()
			client := fluent_test.Open(t, dialect.Postgres, addr, opts)
			defer client.Close()
			for _, tt := range tests {
				name := runtime.FuncForPC(reflect.ValueOf(tt).Pointer()).Name()
				t.Run(name[strings.LastIndex(name, ".")+1:], func(t *testing.T) {
...
var (
	opts = fluent_test.WithMigrateOptions(
		migrate.WithDropIndex(true),
		migrate.WithDropColumn(true),
	)
	tests = [...]func(*testing.T, *fluent.Client){
		Sanity,
		NoSchemaChanges,
		Tx,
		Lock,
...
func Sanity(t *testing.T, client *fluent.Client) {
	...
	require.Equal(child.Name, client.User.Query().Order(fluent.Asc("name")).FirstX(ctx).Name)
	require.Equal(usr2.Name, client.User.Query().Order(fluent.Desc("name")).FirstX(ctx).Name)
	// Update fields.
	...
	require.Error(err)
	require.True(fluent.IsNotFound(err))
	...
	fi, ok := reflect.TypeOf(fluent.Card{}).FieldByName("Edges")
	require.True(ok)
	require.NotEmpty(fi.Tag.Get("mashraki"))
	fi, ok = reflect.TypeOf(fluent.Card{}).FieldByName("ID")
	require.True(ok)
	require.Equal("-", fi.Tag.Get("json"))
	fi, ok = reflect.TypeOf(fluent.Card{}).FieldByName("Number")
	require.True(ok)
	require.Equal("-", fi.Tag.Get("json"))
...
func Upsert(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	...
	id := client.User.Create().
		...
		Update(func(u *fluent.UserUpsert) {
	...
	client.User.CreateBulk(builders...).
		...
		UpdateNewValues().
		ExecX(ctx)
	users := client.User.Query().Order(fluent.Asc(user.FieldPhone)).AllX(ctx)
...
func Clone(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	...
	for i := 0; i < 10; i++ {
		...
		require.True(t, fluent.IsNotFound(err), "should not return syntax error")
	}
}

func Paging(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
	for i := 1; i <= 10; i++ {
	...
	require.Equal(
		...
		client.User.Query().
			Offset(6).
			Limit(2).
			Order(fluent.Asc(user.FieldAge)).
			GroupBy(user.FieldAge).
			IntsX(ctx),
	)
	for i := 0; i < 10; i++ {
		require.Equal(i+1, client.User.Query().Order(fluent.Asc(user.FieldAge)).Offset(i).Limit(1).AllX(ctx)[0].Age)
	}
}

func Select(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	require := require.New(t)
	...
	names := client.User.
		Query().
		Order(fluent.Asc(user.FieldName)).
		Select(user.FieldName).
		StringsX(ctx)
	require.Equal([]string{"bar", "foo"}, names)
	names = client.User.
		Query().
		Order(fluent.Desc(user.FieldName)).
		Select(user.FieldName).
		StringsX(ctx)
	...
	users := client.User.
		...
		Where(user.Name("foo")).
		WithFriends(func(q *fluent.UserQuery) {
	...
	names = client.Pet.Query().Order(fluent.Asc(pet.FieldID)).Select(pet.FieldName).StringsX(ctx)
	require.Equal([]string{"a", "b", "c", "b"}, names)
	names = client.Pet.Query().Order(fluent.Asc(pet.FieldName)).Select(pet.FieldName).StringsX(ctx)
	...
	var (
		...
		p2 = client.Pet.Query().Order(fluent.Asc(pet.FieldID)).AllX(ctx)
	)
	client.Pet.Query().
		Order(fluent.Asc(pet.FieldID)).
		Modify(func(s *sql.Selector) {
	...
	client.Group.Query().
		Order(fluent.Asc(group.FieldID)).
		Modify(func(s *sql.Selector) {
...
func Aggregate(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	...
	owners := []*fluent.User{a8m, nat}
	for i := 1; i <= 10; i++ {
		client.Pet.Create().SetName(fmt.Sprintf("pet%d", i)).SetAge(float64(i)).SetOwner(owners[i%2]).SaveX(ctx)
	}
	s1 := client.Pet.Query().Aggregate(fluent.Sum(pet.FieldAge)).IntX(ctx)
	require.Equal(t, 55, s1)
	s2 := client.Pet.Query().Where(pet.HasOwner()).Aggregate(fluent.Sum(pet.FieldAge)).IntX(ctx)
	require.Equal(t, s1, s2)

	// Aggregate traversals.
	require.Equal(t, 30, a8m.QueryPets().Aggregate(fluent.Sum(pet.FieldAge)).IntX(ctx))
	require.Equal(t, 25, nat.QueryPets().Aggregate(fluent.Sum(pet.FieldAge)).IntX(ctx))
	require.Equal(t, 25, a8m.QuerySpouse().QueryPets().Aggregate(fluent.Sum(pet.FieldAge)).IntX(ctx))
	require.Equal(t, 30, nat.QuerySpouse().QueryPets().Aggregate(fluent.Sum(pet.FieldAge)).IntX(ctx))
...
func ExecQuery(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
...
func NillableRequired(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
...
func Predicate(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
	...
	files := client.File.Query().
		...
		Order(fluent.Asc(file.FieldName)).
		AllX(ctx)
...
func AddValues(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
	...
	require.True(fluent.IsConstraintError(err))
...
func Delete(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
...
func Relation(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
	...
	require.True(fluent.IsNotSingular(err))
	...
	u1 := client.User.Query().Order(fluent.Asc(user.FieldName)).FirstIDX(ctx)
	u2 := client.User.Query().Order(fluent.Desc(user.FieldName)).FirstIDX(ctx)
	require.NotEqual(u1, u2)
	u1 = client.User.Query().Order(fluent.Asc(user.FieldLast), fluent.Asc(user.FieldAge)).FirstIDX(ctx)
	u2 = client.User.Query().Order(fluent.Asc(user.FieldLast), fluent.Desc(user.FieldAge)).FirstIDX(ctx)
	require.NotEqual(u1, u2)
	u1 = client.User.Query().Order(fluent.Asc(user.FieldName, user.FieldAge)).FirstIDX(ctx)
	u2 = client.User.Query().Order(fluent.Asc(user.FieldName, user.FieldAge)).FirstIDX(ctx)
	require.Equal(u1, u2)
	...
	for i, usr := range []*fluent.User{usr, neta} {
	...
	v = v[2:]
	for i, usr := range []*fluent.User{child, child2} {
		require.Equal(usr.Name, v[i].Name)
		require.Equal(usr.Age, v[i].Age)
	...
	client.User.Query().GroupBy(user.FieldName).Aggregate(fluent.As(fluent.Count(), "total")).ScanX(ctx, &v2)
	require.Len(v2, 3)
	for i := range v2 {
...
func ClearFields(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
...
func ClearEdges(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	...
	require.Equal(t, []int{friends[0].ID, friends[1].ID}, a8m.QueryFollowing().Order(fluent.Asc(user.FieldID)).IDsX(ctx))
...
func UniqueConstraint(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
	...
	require.True(fluent.IsConstraintError(err), "c1 already has a parent")
	err = client.User.Create().SetAge(10).SetName("z").SetNickname("z").AddChildren(c2).Exec(ctx)
	require.True(fluent.IsConstraintError(err), "c2 already has a parent")
...
func (m *mocker) rHook() fluent.RollbackHook {
	return func(next fluent.Rollbacker) fluent.Rollbacker {
...
func Tx(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
...
func DefaultValue(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
...
func ImmutableValue(t *testing.T, client *fluent.Client) {
	tests := []struct {
		name    string
		updater func() any
	}{
...
func Sensitive(t *testing.T, client *fluent.Client) {
	require := require.New(t)
	ctx := context.Background()
...
func EagerLoading(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	require := require.New(t)
	...
	t.Run("O2O", func(t *testing.T) {
		...
		require.True(fluent.IsNotLoaded(err))
		require.Nil(pets)
		...
		require.True(fluent.IsNotFound(err), "loaded but was not found")
		require.Nil(parent)
	})
	...
	t.Run("M2O", func(t *testing.T) {
		...
		a8m = client.User.
			...
			WithPets(func(q *fluent.PetQuery) {
				q.WithTeam().Order(fluent.Asc(pet.FieldName))
			}).
			OnlyX(ctx)
	...
	t.Run("M2M", func(t *testing.T) {
		users := client.User.
			Query().
			WithFriends().
			WithGroups(func(q *fluent.GroupQuery) {
				q.Order(fluent.Desc(group.FieldName))
			}).
			Order(fluent.Asc(user.FieldName)).
			AllX(ctx)
		...
		groups := client.Group.
			Query().
			WithUsers(func(q *fluent.UserQuery) {
				q.Order(fluent.Asc(user.FieldName))
			}).
			Order(fluent.Asc(group.FieldName)).
			AllX(ctx)
	...
	t.Run("Graph", func(t *testing.T) {
		users := client.User.
			...
			WithGroups(func(q *fluent.GroupQuery) {
				q.WithInfo()
				q.WithFiles(func(q *fluent.FileQuery) {
					q.WithType()
					q.Order(fluent.Asc(file.FieldName))
				})
				q.Order(fluent.Desc(group.FieldName))
			}).
			Order(fluent.Asc(user.FieldName)).
	...
	t.Run("LimitRows/O2M", func(t *testing.T) {
		...
		users := client.User.Query().WithPets().Order(fluent.Asc(user.FieldID)).AllX(ctx)
	...
	t.Run("LimitRows/M2M", func(t *testing.T) {
		skip(t, "MySQL/5")
		users := client.User.Query().WithGroups().Order(fluent.Asc(user.FieldID)).AllX(ctx)
		require.Len(users[0].Edges.Groups, 2)
		require.Len(users[1].Edges.Groups, 1)
		require.Len(users[2].Edges.Groups, 1)
...
func NamedEagerLoading(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	a8m := client.User.Create().SetName("a8m").SetAge(30).SaveX(ctx)
	...
	a8m = client.User.Query().
		WithNamedPets("Trained", func(q *fluent.PetQuery) { q.Where(pet.Trained(true)) }).
		WithNamedPets("Untrained", func(q *fluent.PetQuery) { q.Where(pet.Trained(false)) }).
		OnlyX(ctx)
	...
	require.True(t, fluent.IsNotLoaded(err))
	require.Nil(t, unknown)

	exists := client.User.Query().
		WithNamedPets("WithSelection", func(q *fluent.PetQuery) {
...
func NoSchemaChanges(t *testing.T, client *fluent.Client) {
...
func Mutation(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	...
	setUsers := func(ms ...*fluent.UserMutation) {
		for _, m := range ms {
			m.SetName("boring")
			m.SetAge(30)
		}
	...
	t.Run("Predicate", func(t *testing.T) {
		...
		updater.SetName("a8m")
		a8m, err = updater.Save(ctx)
		require.True(t, fluent.IsNotFound(err))
		require.Nil(t, a8m)
	})
}
...
var (
	_ = fluent.CardExtension{}
	_ = fluent.Card{}.StaticField
	_ = fluent.Client{}.TemplateField
	_ = []filetype.State{filetype.StateOn, filetype.StateOff}
	_ = []filetype.Type{filetype.TypeJPG, filetype.TypePNG, filetype.TypeSVG}
)

func CreateBulk(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	...
	groups := client.Group.MapCreateBulk(names, func(c *fluent.GroupCreate, i int) {
...
func ConstraintChecks(t *testing.T, client *fluent.Client) {
	var cerr *fluent.ConstraintError
...
func Lock(t *testing.T, client *fluent.Client) {
...
func ExtValueScan(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	u, err := url.Parse("https://github.com/usalko/fluent")
	require.NoError(t, err)
	check := func(ex *fluent.ExValueScan, i *big.Int, u, b64, custom string, ub *url.URL) {
		for _, e := range []*fluent.ExValueScan{ex, client.ExValueScan.GetX(ctx, ex.ID)} {
	...
	u.Path = "/docs"
	ex = ex.Update().SetBinary(u).SetBinaryBytes(u).SetText(big.NewInt(20)).SetBase64("m8a").SetCustom("github.com/usalko/fluent").SaveX(ctx)
	check(ex, big.NewInt(20), u.String(), "m8a", "github.com/usalko/fluent", u)
...
func OrderByFluent(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	users := client.User.CreateBulk(
...
func OrderByEdgeCount(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	users := client.User.CreateBulk(
		client.User.Create().SetName("a").SetAge(1),
		client.User.Create().SetName("b").SetAge(2),
...
// Testing the "low-level" behavior of the sqlgraph package.
// This functionality may be extended to the generated fluent API.
func OrderByEdgeTerms(t *testing.T, client *fluent.Client) {
	ctx := context.Background()
	users := client.User.CreateBulk(
		client.User.Create().SetName("a").SetAge(1),
		client.User.Create().SetName("b").SetAge(2),
...
func drop(t *testing.T, client *fluent.Client) {
