...
type UserAuditLogFunc func(context.Context, *fluent.UserAuditLogMutation) (fluent.Value, error)

// Mutate calls f(ctx, m).
func (f UserAuditLogFunc) Mutate(ctx context.Context, m fluent.Mutation) (fluent.Value, error) {
	if mv, ok := m.(*fluent.UserAuditLogMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *fluent.UserAuditLogMutation", m)
}
