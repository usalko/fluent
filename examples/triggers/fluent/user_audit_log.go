...
func (*UserAuditLog) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case user_audit_log.FieldID:
			values[i] = new(sql.NullInt64)
		case user_audit_log.FieldOperationType, user_audit_log.FieldOperationTime, user_audit_log.FieldOldValue, user_audit_log.FieldNewValue:
...
// Value returns the fluent.Value that was dynamically selected and assigned to the UserAuditLog.
// This includes values selected through modifiers, order, etc.
func (ual *UserAuditLog) Value(name string) (fluent.Value, error) {
...
func (ual *UserAuditLog) Unwrap() *UserAuditLog {
	...
	if !ok {
		panic("fluent: UserAuditLog is not a transactional entity")
	}
