...
type PetUserNameQuery struct {
	config
	ctx        *QueryContext
	order      []pet_user_name.OrderOption
	inters     []Interceptor
...
func (punq *PetUserNameQuery) Order(o ...pet_user_name.OrderOption) *PetUserNameQuery {
...
func (punq *PetUserNameQuery) First(ctx context.Context) (*PetUserName, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{pet_user_name.Label}
	}
	return nodes[0], nil
}
...
func (punq *PetUserNameQuery) Only(ctx context.Context) (*PetUserName, error) {
	...
	case 0:
		return nil, &NotFoundError{pet_user_name.Label}
	default:
		return nil, &NotSingularError{pet_user_name.Label}
	}
}
...
func (punq *PetUserNameQuery) Clone() *PetUserNameQuery {
	...
	return &PetUserNameQuery{
		config:     punq.config,
		ctx:        punq.ctx.Clone(),
		order:      append([]pet_user_name.OrderOption{}, punq.order...),
...
//		GroupBy(pet_user_name.FieldName).
...
func (punq *PetUserNameQuery) GroupBy(field string, fields ...string) *PetUserNameGroupBy {
	...
	grbuild.label = pet_user_name.Label
...
//
//	client.PetUserName.Query().
//		Select(pet_user_name.FieldName).
//		Scan(ctx, &v)
func (punq *PetUserNameQuery) Select(fields ...string) *PetUserNameSelect {
	...
	sbuild.label = pet_user_name.Label
...
func (punq *PetUserNameQuery) prepareQuery(ctx context.Context) error {
	...
	for _, f := range punq.ctx.Fields {
		if !pet_user_name.ValidColumn(f) {
...
func (punq *PetUserNameQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(pet_user_name.Table, pet_user_name.Columns, nil)
	_spec.From = punq.sql
...
func (punq *PetUserNameQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1 := builder.Table(pet_user_name.Table)
	columns := punq.ctx.Fields
	if len(columns) == 0 {
		columns = pet_user_name.Columns
	}
