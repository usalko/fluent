...
func (*CleanUser) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case clean_user.FieldID:
			values[i] = new(sql.NullInt64)
		case clean_user.FieldName, clean_user.FieldPublicInfo:
...
func (cu *CleanUser) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case clean_user.FieldName:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field name", values[i])
			} else if value.Valid {
				cu.Name = value.String
			}
		case clean_user.FieldPublicInfo:
			if value, ok := values[i].(*sql.NullString); !ok {
...
func (cu *CleanUser) Unwrap() *CleanUser {
	...
	if !ok {
		panic("fluent: CleanUser is not a transactional entity")
	}
