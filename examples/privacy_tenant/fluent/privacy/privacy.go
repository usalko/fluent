...
// functions as a query rule.
type TenantQueryRuleFunc func(context.Context, *fluent.TenantQuery) error

// EvalQuery return f(ctx, q).
func (f TenantQueryRuleFunc) EvalQuery(ctx context.Context, q fluent.Query) error {
	if q, ok := q.(*fluent.TenantQuery); ok {
		return f(ctx, q)
	}
	return Denyf("fluent/privacy: unexpected query type %T, expect *fluent.TenantQuery", q)
}

// The TenantMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type TenantMutationRuleFunc func(context.Context, *fluent.TenantMutation) error

// EvalMutation calls f(ctx, m).
func (f TenantMutationRuleFunc) EvalMutation(ctx context.Context, m fluent.Mutation) error {
	if m, ok := m.(*fluent.TenantMutation); ok {
		return f(ctx, m)
	}
	return Denyf("fluent/privacy: unexpected mutation type %T, expect *fluent.TenantMutation", m)
}
