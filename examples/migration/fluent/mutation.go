...
func (m *CardMutation) AddedIDs(name string) []fluent.Value {
	...
	case card.EdgePayments:
		ids := make([]fluent.Value, 0, len(m.payments))
		for id := range m.payments {
			ids = append(ids, id)
		}
		return ids
...
func (m *CardMutation) RemovedIDs(name string) []fluent.Value {
	switch name {
	case card.EdgePayments:
		ids := make([]fluent.Value, 0, len(m.removedpayments))
		for id := range m.removedpayments {
			ids = append(ids, id)
		}
		return ids
...
var _ fluent.Mutation = (*PaymentMutation)(nil)
...
// return value indicates that this field was not set, or was not defined in the
// schema.
func (m *PaymentMutation) Field(name string) (fluent.Value, bool) {
	switch name {
...
// returned if the mutation operation is not UpdateOne, or the query to the
// database failed.
func (m *PaymentMutation) OldField(ctx context.Context, name string) (fluent.Value, error) {
	switch name {
	case payment.FieldCardID:
...
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *PaymentMutation) SetField(name string, value fluent.Value) error {
	switch name {
	case payment.FieldCardID:
		v, ok := value.(int)
		if !ok {
...
// with the given name. The second boolean return value indicates that this field
// was not set, or was not defined in the schema.
func (m *PaymentMutation) AddedField(name string) (fluent.Value, bool) {
	switch name {
	case payment.FieldAmount:
...
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *PaymentMutation) AddField(name string, value fluent.Value) error {
	switch name {
	case payment.FieldAmount:
		v, ok := value.(float64)
		if !ok {
...
// AddedIDs returns all IDs (to other nodes) that were added for the given edge
// name in this mutation.
func (m *PaymentMutation) AddedIDs(name string) []fluent.Value {
	switch name {
...
// RemovedIDs returns all IDs (to other nodes) that were removed for the edge with
// the given name in this mutation.
func (m *PaymentMutation) RemovedIDs(name string) []fluent.Value {
	return nil
}
...
var _ fluent.Mutation = (*SessionDeviceMutation)(nil)
...
// with the given name. The second boolean return value indicates that this field
// was not set, or was not defined in the schema.
func (m *SessionDeviceMutation) AddedField(name string) (fluent.Value, bool) {
	return nil, false
}
...
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *SessionDeviceMutation) AddField(name string, value fluent.Value) error {
	switch name {
	}
...
func (m *UserMutation) AddedIDs(name string) []fluent.Value {
	switch name {
	case user.EdgeCards:
		ids := make([]fluent.Value, 0, len(m.cards))
		for id := range m.cards {
			ids = append(ids, id)
		}
		return ids
...
func (m *UserMutation) RemovedIDs(name string) []fluent.Value {
	switch name {
	case user.EdgeCards:
		ids := make([]fluent.Value, 0, len(m.removedcards))
		for id := range m.removedcards {
			ids = append(ids, id)
		}
		return ids
