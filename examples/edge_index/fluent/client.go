...
import (
	...
	"github.com/usalko/fluent/examples/edge_index/fluent/migrate"
...
func (c *CityClient) mutate(ctx context.Context, m *CityMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("fluent: unknown City mutation op: %q", m.Op())
	}
}
...
func (c *StreetClient) mutate(ctx context.Context, m *StreetMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("fluent: unknown Street mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		City, Street []fluent.Hook
	}
	inters struct {
		City, Street []fluent.Interceptor
	}
)
