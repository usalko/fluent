	...
	tests := []struct {
		...
		},
		{
			...
			before: func(mock pgMock) {
				...
				mock.ExpectExec(escape(`CREATE TABLE IF NOT EXISTS "fluent_types"("id" bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL, "type" varchar UNIQUE NOT NULL, PRIMARY KEY("id"))`)).
					WillReturnResult(sqlmock.NewResult(0, 1))
				mock.tableExists("users", false)
				mock.ExpectExec(escape(`CREATE TABLE IF NOT EXISTS "users"("id" bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL, PRIMARY KEY("id"))`)).
					WillReturnResult(sqlmock.NewResult(0, 1))
				// set users id range.
				mock.ExpectExec(escape(`INSERT INTO "fluent_types" ("type") VALUES ($1)`)).
					WithArgs("users").
					WillReturnResult(sqlmock.NewResult(0, 1))
		...
		},
		{
			...
			before: func(mock pgMock) {
				...
				mock.ExpectQuery(`SELECT "type" FROM "fluent_types" ORDER BY "id" ASC`).
