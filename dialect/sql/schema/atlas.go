...
func (a *Atlas) NamedDiff(ctx context.Context, name string, tables ...*Table) error {
	...
	if a.driver != nil {
		...
		defer c.Close()
		a.sqlDialect, err = a.entDialect(ctx, fluent_sql.OpenDB(a.dialect, c.DB))
		if err != nil {
			return err
		}
		a.atDriver = c.Driver
...
func (a *Atlas) loadTypes(ctx context.Context, conn dialect.ExecQuerier) ([]string, error) {
	...
	rows := &fluent_sql.Rows{}
	query, args := fluent_sql.Dialect(a.dialect).
		Select("type").From(fluent_sql.Table(TypeTable)).OrderBy(fluent_sql.Asc("id")).Query()
	if err := conn.Query(ctx, query, args, rows); err != nil {
		return nil, fmt.Errorf("query types table: %w", err)
	}
	defer rows.Close()
	var types []string
	if err := fluent_sql.ScanSlice(rows, &types); err != nil {
		return nil, err
	}
	return types, nil
}
