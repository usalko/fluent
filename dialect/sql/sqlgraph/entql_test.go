...
func TestGraph_EvalP(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			p:         fluent_ql.FieldHasPrefix("name", "a"),
		...
		{
			s:         sql.Dialect(dialect.Postgres).Select().From(sql.Table("users")),
			p:         fluent_ql.EQ(fluent_ql.F("name"), fluent_ql.F("last")),
		...
		},
		{
			...
			p:         fluent_ql.HasEdgeWith("groups", fluent_ql.Or(fluent_ql.FieldEQ("name", "GitHub"), fluent_ql.FieldEQ("name", "GitLab"))),
		...
		{
			s:         sql.Dialect(dialect.Postgres).Select().From(sql.Table("users")).Where(sql.EQ("active", true)),
			p:         fluent_ql.And(fluent_ql.HasEdge("pets"), fluent_ql.HasEdge("groups"), fluent_ql.EQ(fluent_ql.F("name"), fluent_ql.F("uid"))),
		...
		{
			s: sql.Dialect(dialect.Postgres).Select().From(sql.Table("users")).Where(sql.EQ("active", true)),
			p: fluent_ql.HasEdgeWith("pets", fluent_ql.FieldEQ("name", "pedro"), WrapFunc(func(s *sql.Selector) {
