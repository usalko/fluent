...
// configuration as described in: https://github.com/usalko/fluent/docs/migrate/#universal-ids.
...
//
//	c.Noder(ctx, id)
//	c.Noder(ctx, id, fluent.WithNodeType(typeResolver))
func (c *Client) Noder(ctx context.Context, id string, opts ...NodeOption) (_ Noder, err error) {
	defer func() {
		if IsNotFound(err) {
			err = multierror.Append(err, fluent_gql.ErrNodeNotFound(id))
		}
	}()
...
func (c *Client) Noders(ctx context.Context, ids []string, opts ...NodeOption) ([]Noder, error) {
	...
	for i, id := range ids {
		if errors[i] == nil {
			...
			errors[i] = fluent_gql.ErrNodeNotFound(id)
		} else if IsNotFound(errors[i]) {
			errors[i] = multierror.Append(errors[i], fluent_gql.ErrNodeNotFound(id))
		}
