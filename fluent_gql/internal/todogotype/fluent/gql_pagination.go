...
type (
	Cursor         = fluent_gql.Cursor[string]
	PageInfo       = fluent_gql.PageInfo[string]
...
func (p *billproductPager) applyCursors(query *BillProductQuery, after, before *Cursor) (*BillProductQuery, error) {
	...
	for _, predicate := range fluent_gql.CursorsPredicate(after, before, DefaultBillProductOrder.Field.column, p.order.Field.column, direction) {
...
func (p *categoryPager) applyCursors(query *CategoryQuery, after, before *Cursor) (*CategoryQuery, error) {
	idDirection := fluent_gql.OrderDirectionAsc
	if p.reverse {
		idDirection = fluent_gql.OrderDirectionDesc
	}
...
func (p *friendshipPager) applyCursors(query *FriendshipQuery, after, before *Cursor) (*FriendshipQuery, error) {
	...
	for _, predicate := range fluent_gql.CursorsPredicate(after, before, DefaultFriendshipOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}
...
type FriendshipOrderField struct {
	...
	Value    func(*Friendship) (fluent.Value, error)
	column   string // field or computed.
...
var DefaultFriendshipOrder = &FriendshipOrder{
	...
	Field: &FriendshipOrderField{
		Value: func(f *Friendship) (fluent.Value, error) {
			return f.ID, nil
		},
...
func (p *groupPager) applyCursors(query *GroupQuery, after, before *Cursor) (*GroupQuery, error) {
	...
	for _, predicate := range fluent_gql.CursorsPredicate(after, before, DefaultGroupOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}
...
type GroupOrderField struct {
	...
	Value    func(*Group) (fluent.Value, error)
	column   string // field or computed.
...
var DefaultGroupOrder = &GroupOrder{
	...
	Field: &GroupOrderField{
		Value: func(gr *Group) (fluent.Value, error) {
			return gr.ID, nil
		},
...
func (p *petPager) applyCursors(query *PetQuery, after, before *Cursor) (*PetQuery, error) {
	...
	for _, predicate := range fluent_gql.CursorsPredicate(after, before, DefaultPetOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}
...
type PetOrderField struct {
	...
	Value    func(*Pet) (fluent.Value, error)
	column   string // field or computed.
...
var DefaultPetOrder = &PetOrder{
	Direction: fluent_gql.OrderDirectionAsc,
	Field: &PetOrderField{
		Value: func(pe *Pet) (fluent.Value, error) {
			return pe.ID, nil
		},
		column: pet.FieldID,
		toTerm: pet.ByID,
...
func (p *userPager) applyCursors(query *UserQuery, after, before *Cursor) (*UserQuery, error) {
	...
	for _, predicate := range fluent_gql.CursorsPredicate(after, before, DefaultUserOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}
...
var (
	...
	UserOrderFieldGroupsCount = &UserOrderField{
		Value: func(u *User) (fluent.Value, error) {
...
type UserOrderField struct {
	...
	Value    func(*User) (fluent.Value, error)
	column   string // field or computed.
