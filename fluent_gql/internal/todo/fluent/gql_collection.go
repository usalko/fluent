		...
		case "todos":
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := fluent_gql.LimitPerRow(category.TodosColumn, limit, pager.orderExpr(query))
		...
		case "subCategories":
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := fluent_gql.LimitPerRow(category.SubCategoriesPrimaryKey[0], limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
	...
	if v, ok := rv[orderByField]; ok {
		...
		case []any:
			...
			for i := range v {
				...
				var (
					err1, err2 error
					order      = &CategoryOrder{Field: &CategoryOrderField{}, Direction: fluent_gql.OrderDirectionAsc}
				)
		...
		case "users":
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := fluent_gql.LimitPerRow(group.UsersPrimaryKey[1], limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
...
func newOneToManyPaginateArgs(rv map[string]any) *onetomanyPaginateArgs {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]any:
			var (
				err1, err2 error
				order      = &OneToManyOrder{Field: &OneToManyOrderField{}, Direction: fluent_gql.OrderDirectionAsc}
			)
		...
		case "todos":
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := fluent_gql.LimitPerRow(project.TodosColumn, limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
		...
		case "children":
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := fluent_gql.LimitPerRow(todo.ChildrenColumn, limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
	...
	if v, ok := rv[orderByField]; ok {
		...
		case []any:
			...
			for i := range v {
				...
				var (
					err1, err2 error
					order      = &TodoOrder{Field: &TodoOrderField{}, Direction: fluent_gql.OrderDirectionAsc}
				)
				if d, ok := mv[directionField]; ok {
					err1 = order.Direction.UnmarshalGQL(d)
				}
		...
		case "groups":
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := fluent_gql.LimitPerRow(user.GroupsPrimaryKey[0], limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
		...
		case "friends":
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := fluent_gql.LimitPerRow(user.FriendsPrimaryKey[0], limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
		...
		case "friendships":
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := fluent_gql.LimitPerRow(user.FriendshipsColumn, limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
...
func newUserPaginateArgs(rv map[string]any) *userPaginateArgs {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]any:
			var (
				err1, err2 error
				order      = &UserOrder{Field: &UserOrderField{}, Direction: fluent_gql.OrderDirectionAsc}
			)
			if d, ok := v[directionField]; ok {
				err1 = order.Direction.UnmarshalGQL(d)
			}
