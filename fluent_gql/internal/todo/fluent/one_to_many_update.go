...
func (otmu *OneToManyUpdate) check() error {
	if v, ok := otmu.mutation.Name(); ok {
		if err := one_to_many.NameValidator(v); err != nil {
...
func (otmu *OneToManyUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(one_to_many.Table, one_to_many.Columns, sqlgraph.NewFieldSpec(one_to_many.FieldID, field.TypeInt))
	...
	if value, ok := otmu.mutation.Name(); ok {
		_spec.SetField(one_to_many.FieldName, field.TypeString, value)
	}
	if value, ok := otmu.mutation.Field2(); ok {
		_spec.SetField(one_to_many.FieldField2, field.TypeString, value)
	}
	if otmu.mutation.Field2Cleared() {
		_spec.ClearField(one_to_many.FieldField2, field.TypeString)
	}
	if otmu.mutation.ParentCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   one_to_many.ParentTable,
			Columns: []string{one_to_many.ParentColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(one_to_many.FieldID, field.TypeInt),
			},
		}
	...
	if otmu.mutation.ChildrenCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   one_to_many.ChildrenTable,
			Columns: []string{one_to_many.ChildrenColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(one_to_many.FieldID, field.TypeInt),
			},
