...
func (e OneToManyEdges) ParentOrErr() (*OneToMany, error) {
	if e.Parent != nil {
		...
		return nil, &NotFoundError{label: one_to_many.Label}
	}
...
func (*OneToMany) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case one_to_many.FieldID, one_to_many.FieldParentID:
			values[i] = new(sql.NullInt64)
		case one_to_many.FieldName, one_to_many.FieldField2:
...
// Value returns the fluent.Value that was dynamically selected and assigned to the OneToMany.
// This includes values selected through modifiers, order, etc.
func (otm *OneToMany) Value(name string) (fluent.Value, error) {
...
func (otm *OneToMany) Unwrap() *OneToMany {
	...
	if !ok {
		panic("fluent: OneToMany is not a transactional entity")
	}
