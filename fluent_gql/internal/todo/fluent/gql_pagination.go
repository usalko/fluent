...
func (p *onetomanyPager) applyCursors(query *OneToManyQuery, after, before *Cursor) (*OneToManyQuery, error) {
	...
	for _, predicate := range fluent_gql.CursorsPredicate(after, before, DefaultOneToManyOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}
...
func (p *projectPager) applyCursors(query *ProjectQuery, after, before *Cursor) (*ProjectQuery, error) {
	...
	for _, predicate := range fluent_gql.CursorsPredicate(after, before, DefaultProjectOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}
...
type ProjectOrderField struct {
	...
	Value    func(*Project) (fluent.Value, error)
	column   string // field or computed.
...
var DefaultProjectOrder = &ProjectOrder{
	...
	Field: &ProjectOrderField{
		Value: func(pr *Project) (fluent.Value, error) {
			return pr.ID, nil
		},
...
func (p *organizationPager) applyCursors(query *WorkspaceQuery, after, before *Cursor) (*WorkspaceQuery, error) {
	...
	for _, predicate := range fluent_gql.CursorsPredicate(after, before, DefaultOrganizationOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}
...
type OrganizationOrderField struct {
	...
	Value    func(*Organization) (fluent.Value, error)
	column   string // field or computed.
...
var DefaultOrganizationOrder = &OrganizationOrder{
	...
	Field: &OrganizationOrderField{
		Value: func(w *Organization) (fluent.Value, error) {
			return w.ID, nil
		},
