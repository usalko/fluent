...
func (*BillProduct) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case bill_product.FieldID, bill_product.FieldQuantity:
			values[i] = new(sql.NullInt64)
		case bill_product.FieldName, bill_product.FieldSku:
...
// Value returns the fluent.Value that was dynamically selected and assigned to the BillProduct.
// This includes values selected through modifiers, order, etc.
func (bp *BillProduct) Value(name string) (fluent.Value, error) {
...
func (bp *BillProduct) Unwrap() *BillProduct {
	...
	if !ok {
		panic("fluent: BillProduct is not a transactional entity")
	}
