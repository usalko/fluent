...
func (s *todoTestSuite) TestPaginationFiltering() {
	...
	s.Run("EmptyFilter", func() {
		...
		err := s.Post(query, &rsp)
		s.NoError(err)
		s.Equal(s.fluent.Todo.Query().CountX(context.Background()), rsp.Todos.TotalCount)
	})
...
func (s *todoTestSuite) TestFilteringWithCustomPredicate() {
	ctx := context.Background()
	td1 := s.fluent.Todo.Create().
		SetStatus(todo.StatusCompleted).
		SetText("test1").
		SetCreatedAt(time.Now().
			Add(48 * time.Hour)).
		SaveX(ctx)
	td2 := s.fluent.Todo.Create().
		SetStatus(todo.StatusCompleted).
		SetText("test2").
		SetCreatedAt(time.Now().Add(-48 * time.Hour)).
		SaveX(ctx)
	td3 := s.fluent.Todo.Create().
		SetStatus(todo.StatusCompleted).
		SetText("test2").
		SetCreatedAt(time.Now()).
		SaveX(ctx)
	td4 := s.fluent.Todo.Create().
		SetStatus(todo.StatusCompleted).
		SetText("test3").
		SetCreatedAt(time.Now().Add(-48*time.Hour)).
...
func (s *todoTestSuite) TestNodeOptions() {
	ctx := context.Background()
	td := s.fluent.Todo.Create().SetText("text").SetStatus(todo.StatusInProgress).SaveX(ctx)
...
func (s *todoTestSuite) TestQueryJSONFields() {
	var (
		ctx = context.Background()
		cat = s.fluent.Category.Create().SetText("Disabled").SetStatus(category.StatusDisabled).SetStrings([]string{"a", "b"}).SetText("category").SaveX(ctx)
		rsp struct {
			Node struct {
				Text    string
				Strings []string
...
func TestMutation_CreateCategory(t *testing.T) {
	ec := fluent_test.Open(t, dialect.SQLite,
		fmt.Sprintf("file:%s?mode=memory&cache=shared&_fk=1", t.Name()),
		fluent_test.WithMigrateOptions(migrate.WithGlobalUniqueID(true)),
	)
	srv := handler.NewDefaultServer(gen.NewSchema(ec))
	srv.Use(fluent_gql.Transactioner{TxOpener: ec})
	gqlc := client.New(srv)
