...
var (
	...
	ErrRelaySpecDisabled = errors.New(" fluent_gql: must enable relay specification via the WithRelaySpec option")
		...
		if e.genSchema && !ant.Skip.Is(SkipType) {
			...
			if def != nil {
				if s.Types[def.Name] != nil {
					return fmt.Errorf("found the GQL type conflict for the node %s, please use the fluent_gql.Type() annotation to rename the GQL type", node.Name)
				}
				s.AddTypes(def)
...
func (e *schemaGenerator) buildType(t *gen.Type, ant *Annotation, gqlType, pkg string) (*ast.Definition, error) {
	...
	for _, edge := range t.Edges {
		...
		if ant.RelayConnection && edge.Unique {
			return nil, fmt.Errorf(" fluent_gql: RelayConnection cannot be defined on Unique edge: %s.%s", t.Name, edge.Name)
		}
...
func (e *schemaGenerator) buildEdge(node *gen.Type, edge *gen.Edge, edgeAnt *Annotation) ([]*ast.FieldDefinition, error) {
	...
	for _, name := range mappings {
		...
		case edgeAnt.RelayConnection:
			...
			if !ant.RelayConnection {
				return nil, fmt.Errorf(" fluent_gql.RelayConnection() must be set on entity %q in order to define %q.%q as Relay Connection", edge.Type.Name, node.Name, edge.Name)
			}
...
func (e *schemaGenerator) typeFromField(gqlType string, f *gen.Field, ant *Annotation) (*ast.Type, error) {
	...
	case t == field.TypeJSON:
		return nil, fmt.Errorf(" fluent_gql: json type not implemented without setting an fluent_gql.Type() annotation")
	case t == field.TypeOther:
		return nil, fmt.Errorf(" fluent_gql: other type must have typed defined")
	default:
		return nil, fmt.Errorf(" fluent_gql: unexpected type: %s", t.String())
	}
}

// mapScalar provides maps an fluent.Schema type into GraphQL scalar type.
