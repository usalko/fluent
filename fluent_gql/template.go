...
var (
	...
	// TransactionTemplate adds support for fluent.Client for opening transactions for the transaction
	...
	// TemplateFuncs contains the extra template functions used by fluent_gql.
...
func gqlIDType(nodes []*gen.Type, defaultType *field.TypeInfo) (*idType, error) {
	...
	for _, n := range nodes {
		...
		if !n.ID.HasGoType() || !n.ID.Type.RType.Implements(marshalerType) || !n.ID.Type.RType.Implements(unmarshalerType) {
			return nil, errors.New(" fluent_gql: mixed id types must be type string or implement the graphql.Marshaller/graphql.Unmarshaller interfaces")
		}
	}
	return &idType{
		Mixed: true,
...
func (o *OrderTerm) VarName() (string, error) {
	...
	default:
		return "", fmt.Errorf(" fluent_gql: invalid order term %v", o)
	}
}
...
func orderFields(n *gen.Type) ([]*OrderTerm, error) {
	...
	for _, f := range fields {
		...
		case !f.Type.Comparable():
			return nil, fmt.Errorf(" fluent_gql: ordered field %s.%s must be comparable", n.Name, f.Name)
		default:
	...
	for _, e := range n.Edges {
		...
		case ant.OrderField == fmt.Sprintf("%s_COUNT", name):
			...
			if _, err := e.OrderCountName(); err != nil {
				return nil, fmt.Errorf(" fluent_gql: invalid order field %s defined on edge %s.%s: %w", ant.OrderField, n.Name, e.Name, err)
			}
			terms = append(terms, &OrderTerm{
				Owner: n,
				GQL:   ant.OrderField,
		...
		case strings.HasPrefix(ant.OrderField, name+"_"):
			...
			if idx == -1 {
				return nil, fmt.Errorf(" fluent_gql: order field %s defined on edge %s.%s was not found on its reference", ant.OrderField, n.Name, e.Name)
			}
			terms = append(terms, &OrderTerm{
				Owner: n,
				GQL:   ant.OrderField,
		...
		default:
			return nil, fmt.Errorf(" fluent_gql: invalid order field defined on edge %s.%s", n.Name, e.Name)
		}
	}
	return terms, nil
}
