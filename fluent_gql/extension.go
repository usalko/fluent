...
type (
	// Extension implements the flc.Extension for providing GraphQL integration.
	Extension struct {
		schemaGenerator
...
//	 - fluent.graphql	  // generated schema.
...
// WithTemplates overrides the default templates ( fluent_gql.AllTemplates)
...
// https://github.com/fluent/fluent/issues/1000#issuecomment-735663175.
...
// maps an fluent.Field (*gen.Field) into its GraphQL scalar type. If the
// function returns an empty string, the extension fallbacks to its
// default mapping.
//
//	ex, err := fluent_gql.NewExtension(
//		 fluent_gql.WithMapScalarFunc(func(f *gen.Field, op gen.Op) string {
...
func (e *Extension) Options() []flc.Option {
	return []flc.Option{
		flc.FeatureNames(gen.FeatureNamedEdges.Name),
	}
}
