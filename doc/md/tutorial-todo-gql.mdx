...
[contrib/ fluent_gql](https://pkg.go.dev/github.com/usalko/fluent/fluent_gql) Ent extension and use it for generating our first schema. Then,
...
#### Install and configure ` fluent_gql`

1\. Install ` fluent_gql`:

```shell
go get github.com/usalko/fluent/fluent_gql@master
```

2\. Add the following annotations to the `Todo` schema to enable `Query` and `Mutation` (creation) capabilities:

```go title="fluent/schema/todo.go" {3-4}
...
3\. Create a new Go file named `ent/flc.go`, and paste the following content:
...
func main() {
	ex, err := fluent_gql.NewExtension(
		...
		// the Ent schema in a file named fluent.graphql.
...
```
:::note
The `ent/flc.go` is ignored using a build tag, and it is executed by the `go generate` command through the `generate.go`
file.
:::
...
After installing and configuring ` fluent_gql`, it is time to execute the codegen:
...
3\. Edit the `ent/flc.go` to let Ent know about the `gqlgen` configuration:
...
Now, we're ready to run `go generate` to trigger `fluent` and `gqlgen` code generation. Execute the following command from
...
func main() {
	// Create fluent.Client and run the schema migration.
	client, err := fluent.Open(dialect.SQLite, "file:ent?mode=memory&cache=shared&_fk=1")
	if err != nil {
		log.Fatal("opening fluent client", err)
	}
...
![tutorial-todo-playground](https://github.com/usalko/fluent/images/assets/tutorial-gql-playground.png)
...
func (r *queryResolver) Todos(ctx context.Context) ([]*fluent.Todo, error) {
