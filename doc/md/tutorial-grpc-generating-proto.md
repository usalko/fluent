...
//go:generate go run -mod=mod github.com/usalko/fluent/fluentproto/cmd/fluentproto -path ./schema
```
...
option go_package = "fluent-grpc-example/fluent/proto/fluentpb";
...
```go title="fluent/proto/fluentpb/generate.go"
package fluentpb
...
- To install `protoc-gen-entgrpc`, run:
  ...
  ```
  go install github.com/usalko/fluent/fluentproto/cmd/protoc-gen-entgrpc@master
  ```
...
Observe that a new file named `ent/proto/fluentpb/fluentpb.pb.go` was created which contains the generated Go structs for our entities.
