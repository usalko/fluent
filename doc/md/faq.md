...
```go
// AuditMixin implements the fluent.Mixin for sharing
...
func (AuditMixin) Fields() []fluent.Field {
	return []fluent.Field{
...
func (AuditMixin) Hooks() []fluent.Hook {
	return []fluent.Hook{
		hooks.AuditHook,
	}
}
...
func AuditHook(next fluent.Mutator) fluent.Mutator {
	...
	return fluent.MutateFunc(func(ctx context.Context, m fluent.Mutation) (fluent.Value, error) {
		...
		switch op := m.Op(); {
		case op.Is(fluent.OpCreate):
			ml.SetCreatedAt(time.Now())
			if _, exists := ml.CreatedBy(); !exists {
				ml.SetCreatedBy(usr.ID)
			}
		case op.Is(fluent.OpUpdateOne | fluent.OpUpdate):
...
issue-tracker for more advance examples like [issue-842](https://github.com/usalko/fluent/issues/842#issuecomment-707896368).
...
// Hooks of the Mixin.
func (BaseMixin) Hooks() []fluent.Hook {
	return []fluent.Hook{
		hook.On(IDHook(), fluent.OpCreate),
	}
}

func IDHook() fluent.Hook {
...
In order to use extended identifiers (XIDs) with gqlgen, follow the configuration mentioned in the [issue tracker](https://github.com/usalko/fluent/issues/1526#issuecomment-831034884).
...
func (Location) Fields() []fluent.Field {
	return []fluent.Field{
		field.String("name"),
...
A full example exists in the [example repository](https://github.com/a8m/fluentspatial).
...
[example](https://github.com/usalko/fluent/blob/dd4792f5b30bdd2db0d9a593a977a54cb3f0c1ce/examples/flc_pkg/fluent/template/static.tmpl).
...
example on [GitHub](https://github.com/usalko/fluent/tree/master/examples/flc_pkg).
...
We add receiver methods to the generated protobuf struct such that it implements [ValueScanner](https://pkg.go.dev/github.com/usalko/fluent/schema/field#ValueScanner)
...
func (Message) Fields() []fluent.Field {
	return []fluent.Field{
		field.Bytes("hi").
			GoType(&pb.Hi{}),
	}
...
The [`fluent_sql.Annotation`](schema-annotations.md) option allows adding custom `CHECK` constraints to the `CREATE TABLE`
...
func (User) Annotations() []schema.Annotation {
	return []schema.Annotation{
		&fluent_sql.Annotation{
			...
			Check: "website <> 'github.com/usalko/fluent'",
...
func main() {
	...
	client := fluent.NewClient(fluent.Driver(&multiDriver{w: wd, r: rd}))
	defer client.Close()
	// Use the client here.
}
...
func (d *multiDriver) Query(ctx context.Context, query string, args, v any) error {
	...
	if fluent.QueryFromContext(ctx) == nil {
		e = d.w
	}
