...
![re-fields-properties](https://github.com/usalko/fluent/images/assets/er_fields_properties.png)
...
default fluent type with a custom one.

The custom type must be either a type that is convertible to the Go basic type, a type that implements the
[ValueScanner](https://pkg.go.dev/github.com/usalko/fluent/schema/field?tab=doc#ValueScanner) interface, or has an
...
[`fluent_sql.Annotation`](https://pkg.go.dev/github.com/usalko/fluent@master/dialect/fluent_sql#Annotation):
...
func (User) Fields() []fluent.Field {
	return []fluent.Field{
		...
		field.Time("created_at").
			Default(time.Now).
			Annotations(
				fluent_sql.Default("CURRENT_TIMESTAMP"),
			),
		...
		field.String("field").
			Optional().
			Annotations(
				fluent_sql.DefaultExpr("lower(other_field)"),
			),
...
By default, `fluent` generates the entity model with fields that are configured in the `schema.Fields` method.
...
When a custom [`GoType`](#go-type) is being used, it must be convertible to the basic `string` type or it needs to implement the [ValueScanner](https://pkg.go.dev/github.com/usalko/fluent/schema/field#ValueScanner) interface.

The [EnumValues](https://pkg.go.dev/github.com/usalko/fluent/schema/field#EnumValues) interface is also required by the custom Go type to tell Ent what are the permitted values of the enum. 
...
Implement the [EnumValues](https://pkg.go.dev/github.com/usalko/fluent/schema/field#EnumValues) interface.
```go
package property
...
```
The following example shows how to define an `Enum` field with a custom Go type that is not convertible to `string`, but it implements the [ValueScanner](https://pkg.go.dev/github.com/usalko/fluent/schema/field#ValueScanner) interface: 
...
```
Implement also the [ValueScanner](https://pkg.go.dev/github.com/usalko/fluent/schema/field?tab=doc#ValueScanner) interface.
...
By convention field names should use `snake_case`. The corresponding struct fields generated by `fluent` will follow the Go convention
