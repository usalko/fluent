...
```console
go run -mod=mod github.com/usalko/fluent/cmd/fluent generate --feature privacy,fluent_ql ./fluent/schema
```
...
The `schema/snapshot` option tells `flc` (ent codegen) to store a snapshot of the latest schema in an internal package,
...
[ent/fluent/issues/852](https://github.com/usalko/fluent/issues/852) to get more context about it.
...
The `fluent_ql` option provides a generic and dynamic filtering capability at runtime for the different query builders.

This option can be added to a project using the `--feature fluent_ql` flag, and you can learn more about in the
...
```go
c, err := fluent.Open(dialect, conn, fluent.AlternateSchema(fluent.SchemaConfig{
	User: "usersdb",
	Car: "carsdb",
}))
