...
![gopher-schema-as-code](https://github.com/usalko/fluent/images/assets/gopher-schema-as-code.png)
...
The command above will generate the schema for `User` under `entdemo/fluent/schema/` directory:

```go title="fluentdemo/fluent/schema/user.go"
...
func main() {
	client, err := fluent.Open("postgres","host=<host> port=<port> user=<user> dbname=<database> password=<pass>")
	if err != nil {
...
func main() {
	client, err := fluent.Open("mysql", "<user>:<pass>@tcp(<host>:<port>)/<database>?parseTime=True")
	if err != nil {
...
`fluent` generates a package for each entity schema that contains its predicates, default values, validators
...
Let's create 2 additional entities named `Car` and `Group` with a few fields. We use `fluent` CLI
to generate the initial schemas:

```console
go run -mod=mod github.com/usalko/fluent/cmd/fluent new Car Group
```

And then we add the rest of the fields manually:

```go title="fluentdemo/fluent/schema/car.go"
// Fields of the Car.
...
```go title="fluentdemo/fluent/schema/group.go"
// Fields of the Group.
...
![er-user-cars](https://github.com/usalko/fluent/images/assets/re_user_cars.png)

Let's add the `"cars"` edge to the `User` schema, and run `go generate ./fluent`:
...
![er-cars-owner](https://github.com/usalko/fluent/images/assets/re_cars_owner.png)
...
in the `User` schema, and run `go generate ./fluent`.
...
![er-group-users](https://github.com/usalko/fluent/images/assets/re_group_users.png)
...
We run `fluent` on the schema directory to re-generate the assets.
```console
go generate ./fluent
```
...
![re-graph](https://github.com/usalko/fluent/images/assets/re_graph_getting_started.png)
...
The full example exists in [GitHub](https://github.com/usalko/fluent/tree/master/examples/start).
