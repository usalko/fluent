...
`fluent` supports querying entities with their associations (through their edges). The associated entities
...
![er-group-users](https://github.com/usalko/fluent/images/assets/er_user_pets_groups.png)
...
posts, err := client.Post.Query().
    WithNamedComments("published", func(q *fluent.CommentQuery) {
        q.Where(comment.StatusEQ(comment.StatusPublished))
    })
    WithNamedComments("draft", func(q *fluent.CommentQuery) {
