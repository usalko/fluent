...
As mentioned in the [introduction](code-gen.md) section, running `fluent` on the schemas,
...
pets, err := client.Pet.MapCreateBulk(names, func(c *fluent.PetCreate, i int) {
...
// the operation will return an "fluent.NotFoundError".
...
pets, err := client.Pet.
    Query().
    Select(pet.FieldName).
    WithOwner(func (q *fluent.UserQuery) {
...
However, all builder types implement the generic <a target="_blank" href="https://pkg.go.dev/github.com/usalko/fluent?tab=doc#Mutation">`ent.Mutation`</a> interface.
...
func SetAgeName(m *fluent.UserMutation) {
    m.SetAge(32)
    m.SetName("Ariel")
}
```
