...
func (CleanUser) Annotations() []schema.Annotation {
    return []schema.Annotation{
        ...
        fluent_sql.View(`SELECT name, public_info FROM users`),
    }
}
...
Once defined, you can run `go generate ./fluent` to create the assets needed to interact with this view. For example:
...
Using the `fluent_sql.ViewFor` API, you can use a dialect-aware builder to define the view. Note that you can have multiple
...
func (CleanUser) Annotations() []schema.Annotation {
	return []schema.Annotation{
        fluent_sql.View(`SELECT id, name, public_info FROM users`),
	}
}
...
The full example exists in [Ent repository](https://github.com/usalko/fluent/tree/master/examples/view_schema).
...
data "composite_schema" "app" {
  # Load the fluent schema first with all its tables.
  schema "public" {
...
The full example exists in [Ent repository](https://github.com/usalko/fluent/tree/master/examples/viewcomposite).
...
set it as regular type (`ent.Schema`) and add the `fluent_sql.Skip()` annotation to it to prevent Ent from generating
