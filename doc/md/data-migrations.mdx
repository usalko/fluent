...
```sql title="fluent/migrate/migrations/20221126185750_backfill_data.sql"
...
```go title="fluent/migrate/migratedata/migratedata.go"
package migratedata
...
number_of_files=$(ls fluent/migrate/migrations/*.sql | wc -l)
	...
	return func(next schema.Applier) schema.Applier {
		return schema.ApplyFunc(func(ctx context.Context, conn dialect.ExecQuerier, plan *migrate.Plan) error {
			...
			if hasC {
			    ...
			    // directly on the dialect.ExecQuerier, or use the generated fluent.Client.
			    //highlight-info-end

			    // Create a temporary client from the migration connection.
			    client := fluent.NewClient(
			    	fluent.Driver(sql.NewDriver(dbdialect, sql.Conn{ExecQuerier: conn.(*sql.Tx)})),
			    )
			    if err := client.User.
			    	Update().
			    	SetName("Unknown").
